...
def fx_feld(lauf, tms):
    ...
    ul = protokoll(os.path.join(EXP, lauf))["u_lat"]; z = np.load(os.path.join(EXP, lauf, f"zonenkraft_{tms}ms.npz"))
    ...
    return dict(FL=FL, RH=RH, UU=UU, ox=ox, oy=oy, oz=oz, dx=dx, cpf=cpf, cpref=cpref, Nz=Nz)

FXF = {k: fx_feld(l, t) for k, l, t in (("e501", "p375_e", "000501"), ("b501", "p375_b", "000501"))}
...
print("bis zu der der Streifenmittelwert 1-cp0 > 0,10 bleibt; ausserdem 1-cp0 bei 10/30/60 mm ueber z_w.  OF13 | e501 | b501")
for xq in X_ST:
    for ya, yb in Y_ST:
        ...
        for k, F in FXF.items():
            ...
            ja, jb = int(round((ya - F["oy"])/dx)), int(round((yb - F["oy"])/dx))
            jc, jd = int(round((-yb - F["oy"])/dx)), int(round((-ya - F["oy"])/dx))
