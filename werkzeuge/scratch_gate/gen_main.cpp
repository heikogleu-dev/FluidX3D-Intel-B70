...
static string device_defines(const bool elibb, const bool ptrt, const bool rho16, const bool sparsam, const bool u16, const bool rand) {
	...
	"\n#define TS_A";
	s += "\n #define def_nb_stride 2ul"; // ★ 16.09.2026 APG-Umbau: unbedingt emittiert (fac_nachbar_ab steht in jeder Domaene), wie lbm.cpp
	return s;
}
...
int main(int argc, char** argv) {
	...
	if(argc>=4&&(string(argv[1])=="datei"||string(argv[1])=="dateih3")) { // dateih3: zusaetzlich KLEMM_HAKEN3 (Gate-Arm fuer den Negativhaken)
		...
		string pos = "";
		// ★ 16.09.2026 APG-Umbau: fac_nb hat einen emittierten Stride (2, unter FACETTEN_APG 5). Die Dumps vom 15.09. kennen ihn nicht -- nachziehen wie KLEMM_BILANZ oben.
		string nbstride = (defs.find("#define def_nb_stride")==string::npos) ? string("\n #define def_nb_stride 2ul") : string(""); // unbedingt, wie die Emission in lbm.cpp (16.09.)
		string apg = "";
		...
		schreiben:
		const string code = geraet + defs + klemm + huellen + nbstride + apg + pos + get_opencl_c_code();
