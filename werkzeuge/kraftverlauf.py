    ...
    with open(p) as f:
        for row in csv.DictReader(r for r in f if not r.startswith("#")):
            ...
            for k, v in werte.items():
                sp[k].append(v)
    if not t:
        raise SystemExit(f"{lauf}: keine verwertbaren Zeilen")
...
def main(argv):
    ...
    for n, lauf in enumerate(a.laeufe):
        t, sp = lies(lauf)
        ...
        for k, name, stil, seite in GROESSEN:
            ...
            if a.bis is not None:
                m = tr <= a.bis; tr, yr = tr[m], yr[m]
            if len(tr) == 0:
                continue
            spanne += [tr.min(), tr.max()]
            beschr = f"{lauf} {name}" if len(a.laeufe) > 1 else name
    ...
    o.set_title(f"cd_rest und cz_rest, Raster {a.raster:.0f} ms ({art})",
                fontsize=11, color="#eceff4")
