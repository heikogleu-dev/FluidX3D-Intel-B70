...
| `WALL_MODEL_VEHICLE` (CC#10) | ⚠️ **Code compiled but disabled at vehicle cells in current Step-1b Safe-State default.** Phase B 2.5 diagnostic complete (2026-05-13): WW Krüger has **three stable attractors** (BB +1820 N, halved +163k N, full -610 N), none physically correct. Krüger Moving-Wall pattern fundamentally unfit for stationary wall WW. See [`findings/36_phaseB_diagnostic_complete.md`](findings/36_phaseB_diagnostic_complete.md). |
| **Phase C-B Bouzidi Step 1** | ✅ **PASSED 2026-05-13.** Axis-aligned Bouzidi sub-grid bounce-back validated on 2D Poiseuille (5 q-values, L2-errors 0.76–3.51%, all under 5% pass criterion). Bouzidi @ q=0.5 reproduces standard BB exactly. See [`findings/38_bouzidi_implementation_plan.md`](findings/38_bouzidi_implementation_plan.md). Step 2 (per-cell q from STL voxelization) in progress. |
...
| **2** | **Bouzidi Sub-Grid Bounce-Back (Phase C-B)** — interpolated BB using sub-cell wall-distance q ∈ [0, 1]. Reduces ½-cell-discretization error of standard BB. Complementary to WW (not replacement). Sub-grid distance precomputed via ray-cast through STL. | **5-7 days** (revised) | Bouzidi 2001 paper, [OpenLB `BouzidiVelocity`](https://gitlab.com/openlb/release/-/blob/master/src/boundary/bouzidiBoundary.h) | 🟢 **IN PROGRESS — Step 1 PASSED 2026-05-13.** Axis-aligned Bouzidi z-walls validated on 2D Poiseuille (L2 0.76-3.51% all 5 q-values). Step 2 (per-cell q from STL voxelization + sparse storage + generalize to 18 D3Q19 directions) next. Tier-ladder: Tier 1 ✓ Poiseuille → Tier 2 ⏳ Sphere drag → Tier 3 ⏳ Ahmed → Tier 4 ⏳ MR2. |
