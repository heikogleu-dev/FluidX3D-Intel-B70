...
| **5b-DR** | Double-Res Schwarz Coupling (Far @ 20mm, Near @ 10mm) mit Pure-BB | mittel | 🟢 **Hauptpfad next**. Far cells 225M→28M (8× Reduktion), echter Multi-Res Speed-Up. Mode 1 one-way reicht für ersten DR-Test. Implementation needed: bilinear up/downsample in `couple_fields()` (~50-80 LOC). |
| **5b-Refined** | Proper Schwarz mit Outer-Loop + Mass-Flux-Correction | hoch (2-5 Tage) | 🔮 zukünftige Arbeit. Bidirectional sinnvoll erst mit (a) multiple inner iterations pro chunk, (b) mass-balance über jede Coupling-Plane, (c) damped soft-BC blend statt hard TYPE_E. |
