...
Phase 7 (Triple-Res with iGPU-hosted Coarse domain at 24 mm, 18.4 × 9 × 7.6 m, ~83 M cells in 5 GB system RAM, coupled to B70-hosted Far at 12 mm + Near at 4 mm via 3:1:2 cascade) executed its **first production run** (`bin/forces_phase7_run1.csv`, 82 chunks, 17 min wallclock — vs Phase 6C 58 min). Setup worked end-to-end: B70 28.2 GB VRAM allocated (Near+Far), iGPU 5.3 GB shared sys-RAM (Coarse), Mode 3 PERF-G concurrent run_async on all 3 devices, sync correct (Near 150 + Far 50 + Coarse 25 steps/chunk → 1.5 ms SI-Δt). Convergence trigger fired at chunk 82 with |dFx|/Fx = 0.017 %, **but** the trigger is falsely positive: 2-chunk saw-tooth oscillation (σ/μ Fx_near = 58 %) gets averaged out by the 25-chunk sliding window — same pathology as Phase 6D. The result (Cd 1.62) is therefore not trustworthy. Next iteration (Run 2 tomorrow) uses chunk_far=100 instead of 50 to dampen the saw-tooth. Detail: [`findings/PHASE_7_RUN1_2026-05-16.md`](findings/PHASE_7_RUN1_2026-05-16.md).
...
| **5b (same-res Mechanik-Test)** | Two-LBM-instance same-res Schwarz coupling auf single GPU (Far 225M + Near 38.85M @ 10mm) | mittel | ✅ **2026-05-13 ABGESCHLOSSEN**. 4 Coupling-Modi getestet (no coupling, one-way Far→Near, bidirectional band=5/20). Winner: **Mode 1 one-way Far→Near** mit Δ +14.3% Near-vs-Far drag bias. Mode 0 verification: +59.2% bias bei pure reflection → one-way coupling reduziert das um 76% → **Reflektion ist Hauptursache, nicht reine Geometrie.** Naive Bidirectional (Mode 2) fundamentally instabil ohne proper Schwarz outer-loop — Far driftet, Near std explodiert auf 48% bei band=20. Two-LBM-Pattern mechanisch validiert. See [`findings/PHASE_5B_DUAL_DOMAIN_RESULT_2026-05-13.md`](findings/PHASE_5B_DUAL_DOMAIN_RESULT_2026-05-13.md). |
| **5b-DR** | Double-Res Schwarz Coupling (Far @ 15mm + Near @ 5mm, 3:1 ratio) mit Pure-BB | mittel | ✅ **2026-05-14 PASSED (Mode 1)**. Pfad A: Far 190M + Near 242M = 27.7 GB VRAM. PERF-D Optimierung (`sync_pcie=false` für batched sync) → **99% GPU sustained**. Bilinear up/downsample in `couple_fields()` implementiert. Mode 1 one-way: **Fx_far 4835 ±13%, Fx_near 2181 ±2%, Δ -54.9%** — Resolution-Win bestätigt (Near liefert sauberere BL → niedrigere Drag). Mode 2 bidirectional band=2 chunk=25: **INSTABIL** (Feedback-Loop, Near kollabiert auf 844 N ±78%). DR Mode 1 ist validated state. See [`findings/PHASE_5B_DR_RESULT_2026-05-14.md`](findings/PHASE_5B_DR_RESULT_2026-05-14.md). |
| **5b-Refined** | Proper Schwarz mit Outer-Loop + Mass-Flux-Correction | hoch (2-5 Tage) | 🔮 zukünftige Arbeit. Bidirectional sinnvoll erst mit (a) multiple inner iterations pro chunk, (b) mass-balance über jede Coupling-Plane, (c) damped soft-BC blend statt hard TYPE_E. Empirisch: α=0.2 nicht ausreichend bei chunk=25; entweder α≤0.05 oder chunk=100 nötig. |
| **5c** | Triple-Res Extension (3 LBM layers: Far/Mid/Near) | mittel-hoch | 🟢 **next Hauptpfad** nach DR-Mode-1 PASS. 24M + 95M + 240M cells, ideal 3× GPU compute-balance, Pioneer-Pattern für consumer-GPU Multi-Resolution Aero. |
