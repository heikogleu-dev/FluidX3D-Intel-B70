...
|---|---|---|---|---|
| **1** | **True symmetry plane (specular reflection)** — separate post-stream-collide OpenCL kernel that reads/writes PDFs in fluid neighbors of TYPE_Y cells, bypassing Esoteric-Pull. Wall-normal detected from neighbor flags, reflected direction from precomputed `mirrorY[19]` lookup table. Per-direction PDF copy. | **2–3 days** | [waLBerla `FreeSlip::treatDirection()`](https://github.com/lssfau/walberla/blob/master/src/lbm/boundary/FreeSlip.h), OpenLB `addSlipBoundary` | ✅ **Feasible** for plane Y_min (no corners). Maintainer ProjectPhysX in [issue #288](https://github.com/ProjectPhysX/FluidX3D/issues/288) noted *"stuck on the literal corner case in 3D where specular reflection of DDFs reflect them from space diagonals to axis-aligned directions … no solution to this in literature either"* — but our half-domain has only a flat Y_min plane, not corners. waLBerla solves the corner case explicitly with a fallback to bounce-back. |
| **2** | **Ghost-cell mirror at Y_min** — alternative to #1: keep TYPE_E at Y_min but a small post-stream kernel writes `u(y=0) = u(y=1)` with `u_y → -u_y`. Equilibrium-forced TYPE_E cells then naturally produce the symmetric counter-flow via FluidX3D's existing `if(flagsn_bo==TYPE_E)` branch in stream_collide. | **2–4 hours** | none direct, but standard ghost-cell technique in classic FD codes | ✅ **Feasible.** Reuses existing TYPE_E machinery, only adds one extra post-stream kernel. Lower mathematical purity than #1 but trivially compatible with Esoteric-Pull (writes happen to `u`/`rho` arrays, not `fi`). |
| **3** | **Werner-Wengle wall model** — Two-Layer power-law wall function (`u+ = y+` for `y+ ≤ 11.81`, `u+ = 8.3·y+^(1/7)` else). Closed-form `u_τ = (u_t / [8.3·(y/ν)^(1/7)])^(7/8)`. Modifies the velocity used in `apply_moving_boundaries` from no-slip (u=0) to a slip-velocity approximating the unresolved viscous sublayer. | **3–5 days** (reference impl complex) | [OpenLB `WallFunctionBoundaryProcessor3D`](https://github.com/openLB/openLB/blob/master/src/boundary/wallFunctionBoundaryPostProcessors3D.h), `wallProfile=1 (PowerLaw)`, `WallFunctionForcedD3Q19Descriptor` with extra storage `AV_SHEAR, TAU_W, TAU_EFF, FORCE, V12` | ⚠️ **Feasible but invasive.** Requires new per-cell storage (~16 B/cell extra at TYPE_S boundary cells = ~100 MB at vehicle surface). New post-stream kernel computes u_τ from first-fluid-cell tangential velocity, applies van-Driest damping, modifies bounce-back. Maintainer ProjectPhysX has acknowledged the absent wall model is a known limitation ([issue #127](https://github.com/ProjectPhysX/FluidX3D/issues/127): *"expect drag force to be too large by about a factor 2, due to lack of grid refinement and no wall model"*) and a candidate enhancement. |
