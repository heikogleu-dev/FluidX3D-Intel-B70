...
| **4a** | **dx-Umrechnung vervollständigen** (Heikos Entscheid 16.09. 14:58) | **GEBAUT 16.09. 18:00** (Plan PLAN-DX-UMRECHNUNG-2026-09-16.md, Commits 2be8205…dd3fb1f): `schritt_skal = ulat_skal·dx_skal` in `env_schritte`/`zaehl_takt`, Basis-Wächter `schritte_fein` = Wert bleibt, Basis nachgezogen (Längen-Einheiten, SISM/FAR_LX/PERF_AB ergänzt), Serienzeilen auf 4-mm-Werte | **Abnahme läuft:** F1 Kugel-CPU, F2 8 mm bitgleich, F3 4 mm bitgleich (`p4_pu8_dx`), F4 `p375_b` (SISM 150,0 ms). Prüfagent parallel. Danach ERLEDIGT |
