...
**22.09. 17:05 — Plan C (`CFD_SGS_BAND_PI=1`, Π-konsistente SISM-EMA im Band) gemessen und besser als das FD-Band** (Protokoll B39–B42): der Bandabzug nutzt jetzt denselben Schätzer wie der Zähler (S_Π aus dem Nichtgleichgewichts-Tensor statt FD-Stencil; Π/FD in Lage 2 war 1,53, B38). 8 mm `s5_bandpi2_12_8` (eine Variable gegen `s4_band2_12_8`): H Dachplateau **1,613 ± 0,074** (FD-Band 1,715, Bezug 2,019; Pi−FD −0,103 ± 0,036 = 2,8 σ), Saugspitze unverändert, Streuung über 12 Punkte 0,159 → 0,101 → 0,074; cz_rest −0,706 → −0,752 → −0,778, cd_rest zurück auf Bezug (0,433/0,444/0,430); Band-Klemme 6,3 → 13,5 %, Wanduhr und Speicher unverändert, ein Kernelstart weniger. Zwei Prüfdurchgänge: HOCH H1 (Statik nach dem Einfrieren gelesen → Host FD, Kernel Pi, 6× Überlauf; behoben, Kohärenzwächter + Slot-126-Abnahme eingebaut) und M1 (Queue-Skript ignorierte `CFD_QUEUE_DEV` in der Zeile — alle Kanalsprossen liefen auf der iGPU; fahrzeug_dd unberührt; behoben 17:10). **Standard-Kandidat: `CFD_SGS_BAND=2 CFD_SGS_BAND_PI=1 CFD_U_SPARSAM=0` — 4-mm-Bestätigung nach Heikos Go** (Serienzeile `logs/serie_p4_band2_4mm.txt`).

**22.09. 19:50 — q-Facettenthema GESCHLOSSEN (Heiko).** Punkt 1 (Sekanten-Wand `CFD_FAC_QQUELLE=2`) wird nicht gebaut. Vorprüfung dokumentiert (Protokoll B47/B49): q stammt aus dem **Voxelkörper** (Taubin-Remesh über das finale flags-Feld, setup.cpp:2374), **nicht** aus der STL — der Merkzettel „Kein STL-Rückgriff" ist eingehalten; q = 0,5000 exakt an **allen** 15 114 078 rohen Solid-Links (4 mm, p4_neu.log:382), 56,2 % der Links liegen heute in dem Bin, in dem ELIBB bitgleich Bounce-Back ist. Der Bauweg (flächenbilanzierte Sekante, nicht additiv — sonst stirbt der kipp0-Bitanker) und das Abbruchkriterium ohne Zeitschritt (`CFD_FACETTEN_DIAG=2`, 25 s CPU) stehen im Protokoll, falls das Thema wieder aufgemacht wird.

**22.09. 20:00 — Punkt 3 (Rückfall-Zensus) ABGESCHLOSSEN, null Zeilen Code** (Protokoll B50). Der Zensus war seit 04.09. gebaut (`STATISCHER KLASSENZENSUS`, setup.cpp:2895 ff.) und stand im Lauflog. 4 mm: Rang 2 50,79 %, Rang 1 30,66 %, **Rang 0 (nie lösbar) 18,56 %**, ohne Wandlink 0; Flackerfenster 2 Facetten. Gemessene Laufzeit-Rückfallquote **24,5 %** gegen diese statische Untergrenze ⇒ **nur knapp 6 Prozentpunkte sind beeinflussbar**. Ursache benannt: **92 % der Rang-0-Facetten haben genau einen Wandlink** (533 136 = 16,28 %, y_w Median 1,09, 28,5° — die konvexen Treppenecken); ein Link kann zwei Tangentialrichtungen nicht aufspannen. Der Rückfall ist damit **kein Verfahrensproblem**: entweder mehr Links (Auflösung / D3Q27) oder eine andere Behandlung der Ein-Link-Zelle als der Tangentialsolve. Das SGS-Band bewegt den Rückfall nicht (24,0–24,5 % in allen Armen).
...
| **4c** | **y-Halbzellen-Versatz des Fahrzeugs** | **STANDARD seit 16.09. 21:55 (Heiko): Basis `CFD_Y_VERSATZ 1`** — jede dd-Zeile trägt ihn (Wächter erzwingt), Bitgleich-Regressionen (`kl_std.txt`, `p4_pu8_dx`) deklarieren `=0`. Gebaut 91843b7/d9a5dbf, Prüfagent sauber, Membran 82 % → 0,3 % (3,75 mm), 73 % → 0,8 % (8 mm). `p4_register` (4 mm) war um ein ulp zufällig membranfrei → 3,75↔4-mm-Vergleich vom 16.09. nachmittags verunreinigt. `p375_e` (Band 2+3 + Versatz): Cd_rest −4 %, Cz_rest +0,028 ± 0,016 (weniger Abtrieb, 1,8 σ) | Kein weiterer Lauf (Heiko 21:55). Offen: neuer 4-mm-Bezug MIT Versatz erst nach Heikos Go; Band bei 3,75 mm ohne Wirkung Richtung OF13 |
