...
| **4a** | **dx-Umrechnung vervollständigen** (Heikos Entscheid 16.09. 14:58: „Abstände/Maße nicht auflösungsabhängig ändern, nur Physik/Zeiten entsprechend anpassen") | Befund: `env_schritte` (setup.cpp:91) skaliert schrittbasierte Schalter nur mit `ulat_skal()`, nicht mit dx — `CFD_SGS_SISM_AB`/`_T` werden bei 3,75 mm 9,4 ms zu früh scharf (140,6 statt 150,0 ms) | **(a) Zeiten:** `env_schritte` um den Faktor dx_basis/dx_lauf erweitern (SISM_AB 9000→9600, SISM_T 3000→3200, VD_AB, DIAG_AB, SAMPLE_EVERY). **(b) Maße:** Einheit umtragen `CFD_N2F_BAND_N` zellen_grob→zellen_grob_laenge (8→9), `CFD_N2F_BAND_WANDFREI` und `_PLATEAU` modus→zellen_grob_laenge, `CFD_BODEN_EQ_ABSTAND` modus→zellen_fein. **(c)** `SGS_SISM_AB`/`_T`, `FAR_LX`, `PERF_AB` in die Basis aufnehmen. Grenze: bei Zellzahl 2 hält die Diskretisierung die Länge nicht (8,0→7,5 mm) — deklarieren, nicht wegdefinieren. Details Protokoll 15:05. **Bau erst nach `p375_a`** (kein `make` bei laufender Queue) |
