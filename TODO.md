...
| **4a** | **dx-Umrechnung vervollständigen** (Befund 16.09. auf Heikos Sicherheitsfrage) | `env_schritte` (setup.cpp:91) rechnet schrittbasierte Schalter nur nach `u_lat` um, **nicht nach dx** — `CFD_SGS_SISM_AB`/`_T` werden dadurch bei 3,75 mm 9,4 ms früher scharf (140,6 statt 150,0 ms); `CFD_SGS_SISM_AB`/`_T`, `CFD_FAR_LX`, `CFD_PERF_AB` stehen zudem nicht in der Basis und sind damit ungeprüft | `env_schritte` um den dx-Faktor erweitern; die vier Schalter in `basis/fahrzeug_dd.basis` aufnehmen (Einheit `schritte_fein` bzw. `phys`). Details Protokoll 14:36. **Klasse C zur Entscheidung:** `CFD_BODEN_EQ_ABSTAND` und `CFD_N2F_BAND_N` behalten die ZELLZAHL, ihre physikalische Länge wandert mit dx — bei `SPONGE_N`/`WAKE_ABSTAND` ist es umgekehrt |
