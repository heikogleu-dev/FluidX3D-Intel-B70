...
| **4a** | **dx-Umrechnung vervollständigen** (Heikos Entscheid 16.09. 14:58) | **GEBAUT + GEPRÜFT 16.09.** (Plan PLAN-DX-UMRECHNUNG-2026-09-16.md §A–J, Commits 2be8205…17c95b5; Prüfagent: kein offenes HOCH): `schritt_skal = ulat_skal·dx_skal` in `env_schritte`/`zaehl_takt`, Basis-Wächter `schritte_fein` = Wert bleibt, Basis nachgezogen, Serienzeilen auf 4-mm-Werte (+`CFD_FAR_LX`) | **Abnahme:** F1 Kugel-CPU 4/4 Hashes ✓, F2 8 mm bitgleich ✓, F4 `p375_b` SISM 150,0 ms ✓ und Kräfte = p375_a; F3 `p4_pu8_dx` läuft. **Rest (Bau nach der Queue):** Rundungsansage bei Ist == llround(Soll), `isfinite` in `dx_skal_setzen`, zwei Meldungstexte (Fix-Skript liegt bereit) |
