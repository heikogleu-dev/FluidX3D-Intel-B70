...
| **2** | **4-mm-Voll-Lauf mit vollem aktuellem Stand** (`p4_pu`, gestartet 16.09. 09:52) | = p4_neu-Zeile + SPARSAM + `CFD_POSITIV=2 CFD_U_KLEMME=1`. **KEIN neuer Referenzlauf** — der Bezug bleibt `p4_neu`, `basis/` wird NICHT neu erzeugt (Heiko 16.09.: „wir haben einen Bezug"). Reproduzierbarkeits-Punkt (Abschnitt 3) bewusst ZURÜCKGESTELLT (Heiko: vermutlich Voxelierung, später) | Ergebnis gegen `p4_neu` lesen: cd_rest/cz_rest, Band getrennt, Zeitreihentest (Vorzeichenkonstanz) |
