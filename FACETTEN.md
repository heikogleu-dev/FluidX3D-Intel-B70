...
- **Zählerpuffer `rho_clamp_hits`, 22 Slots (0–21; [20] BODEN_EQ-, [21] EINLASS_EQ-Wirkpfad, je t%100):** [0/1] RHO_CLAMP unten/oben ·
...
| `CFD_BODEN_EQ` / `CFD_BODEN_EQ_DOWN` (nah), `CFD_FERN_BODEN_EQ` / `_DOWN` (fern) | **V1-apply_floor_velocity-Port (DER Boden-Fix):** post-stream Equilibrium-Reset (lokales rho) auf z=1..N; stromauf der Nase N, ab Nase DOWN (Heiko: max 3, besser 2 — Code warnt ab N>3 an nah/fern/kugel, R3; DOWN=1 läuft bei 68 mm Freiheit berührungsfrei unterm Wagen; Aufprägung verfälscht Kräfte adjazenter Zellen — deshalb Split). 8-mm-Beleg: Profil 1,216 bei x≈1,29 m (**Nase**, nicht Wagenmitte — dort 1,044 vs OF13-Soll 1,087, XL-3-Etikettkorrektur); Cz −0,68 ist **kontaminationsdominiert** (ABSTAND-A/B 2026-08-21: gleiche Lage sauber gemessen = **+0,18**, das Vorzeichen kippt!) — nur mit Kontaminations-Etikett zitieren. Wirkpfad-Zähler Slot 20 (Ist>0-Pflicht am Laufende). Wirkt auch im Kugel-Messarm (dort uniformes Band ohne Split/DOWN). |
| `CFD_BODEN_EQ_ABSTAND` | **Heiko-Idee 2026-08-20 (Reifenschutz):** Bandzellen mit TYPE_S näher als A Zellen (Chebyshev, gleiche Ebene + oberhalb; Fahrbahn z=0 zählt nicht) werden NICHT aufgeprägt (Aussparung bei Abstand ≤ A) — gegen Kraft-Kontamination reifenadjazenter Zellen. 0 = V1-Verhalten. Gilt nah + fern + kugel. **VALIDIERT 2026-08-21 (8mm-A/B): Strömung identisch, Kräfte um ΔCd −0,36/ΔCz +0,74 verschoben = die Aufprägung erzeugte ~−0,7 KÜNSTLICHEN Abtrieb an den Reifen. ABSTAND=2 ist PFLICHT-Kraftmaß; alle ABSTAND=0-Kraftwerte gelten als kontaminiert.** |
| `CFD_Y_VERSATZ` | **Standard 1 seit 16.09.2026 (Basis).** Versetzt den Voxelkörper um eine halbe FEINE Zelle in +y (fein und grob derselbe physische Versatz), damit die STL-Symmetrieebene y=0 nicht auf einer Zellmitte liegt — dort bildet der Strahl-Paritäts-Voxelizer ein Ein-Zellen-Blech beidseits der Mittelebene (3,75 mm 82,2 %, 8 mm 73,3 % Dicke-1-Wandzellen; 4 mm zufällig frei, 1 ulp). Idempotent (nur bei Zellmitte), Ansage der Lage fein/grob, `werkzeuge/membran_y0.py` misst. 0 = alte Geometrie (Bitgleich-Regressionen deklarieren 0). Nur fahrzeug_dd; fahrzeug/fernfeld melden „nicht angewandt". |
| `CFD_Z_OFFSET_MM` | Fahrzeuglage relativ z=0, **Default −3 (OF13-Referenzlage seit 2026-08-20)**. ACHTUNG Anker-Verschiebung auch bei 8 mm massiv: f_aus (Lage 0) Cd 1,444/Cz +0,062 vs f_zoffm3 (−3) 1,222/+0,208 — die komplette BODEN_EQ-Leiter (f_aus…f_v1port3) lief auf Alt-Lage 0 und ist mit f_neustandard (−3) NICHT direkt vergleichbar (ΔCz≈0,15 allein aus der Lage). ALLE Zahlen dieser Zeile sind zudem ABSTAND=0 = kontaminiert (R1-M2-Etikett). |
| `CFD_FERN_EINLASS_EQ` | **V1-Port apply_inlet_velocity (Heiko 2026-08-21; 8mm-validiert am 21.08.: kraftneutral, Streifen-RMS −11 % solo / −99 % mit Sponge):** Zellen x=1..N hinter dem Far-x−-Einlass jeden Schritt post-stream auf Equilibrium mit lokalem rho. Gegen die vermessenen Einlass-Streaks (λ 4–6 Zellen, 0,5–1,5 % u_∞, erben bis in die Near-Box). Nur Fernfeld/fernfeld-Fall; Nah-x− ist Kopplungsebene und bleibt tabu. Standard N=2 (Heiko; V1: 3), Wirkpfad Slot 21 mit Null-Fehler + Nahfeld-Negativkontrolle. Begleiter: CFD_SPONGE_N — **Zellenzahl, skaliert NICHT physisch mit**: 64 @ 16-mm-Grobgitter ≙ 32 @ 32 mm (8-mm-Fall); der NF_OX-Guard fängt Fehlkonfigurationen hart (kostete am 21.08. zwei Läufe). 8mm-Validierung: Sponge −98 % Streifen-RMS, +einlass_eq −99 %, beide kraftneutral. |
| `CFD_NEAR_VOR_MM` | **Heiko 2026-08-21, VALIDIERT:** Near-Box-Einlass um Δ mm (auf dx_c gerastert; 96 = 6×16 = 3×32) weiter vor die Nase — Variante B: Box wächst nach vorn, Heck-Ende weltfest. Vorstau entwickelt sich im Feingitter statt am Interface geklemmt; 8mm-A/B: −0,04..−0,05 Cz echt, +1,4 % Zellen. **Standard 96.** |
| `CFD_FAC_GPU` / `CFD_FAC_GPU_PRUEF` | **Default AN (=1):** kraft_facetten-Druckprojektion als GPU-Reduktion über Markerzellen-Liste statt 2–2,5-GB-F-Transfer + Host-Dreifachschleife — Kraftfenster-Anteil 35,8 % → 1,3 % (B70-Index-A/B). `=0` = alter Host-Pfad wortgleich (Kontrollarm; gedruckte Facetten-Cd-Werte weichen ~1e-5 relativ, Feld/forces.csv/Anker unberührt). `_PRUEF=1` rechnet beide + druckt Abweichung/Zähler (iGPU: Zähler exakt, 8,45e-6). Zero-Copy-Lehre: run() mit finish, finish vor Host-Spiegel-Reads. |
| `CFD_KRAFT_ZBAND` | **Heiko-Diagnose (Zellen, 0=aus=bitidentisch):** Kraft-Zerlegung Kontaktband (z<N ab z0) vs Rest via object_force_zband + z-gefilterte Facetten-Zweitliste; kraft_zband.csv, Selbsttest Band+Rest=Gesamt (<5e-5, Wächter). GITTERBAND: zwischen DX-Sprossen nicht direkt vergleichbar. 8mm-Kernbefund: Band trägt +0,61 Cz (maskiert Karosserie-Abtrieb −0,47) — **Cz_rest = ehrlichere Karosserie-Metrik.** Scope: Druckanteil (Reibung ohne z, Folgearbeit fac_geo[6]). |
...
| `CFD_FAC_APG` | float κ (Default 0 = aus, bitgleich): tw-Ziel = Spalding − κ·y_ab·dp/ds (seit 16.09. Abtasthöhe y_ab, dp/ds aus dem Vorkernel fac_apg_ab, braucht `CFD_FAC_NACHBAR=1`), Klemmen [0, 2·tw] (Slot 19 zählt BEIDE, [309]/[310] getrennt). Nur Arme 3/4; mit PEMA gesperrt. **Gemessen 16.09./22.09. (8 mm, κ=1): Autorität ≥1 in 97,1 %, 97,06 % geklemmt — Vorzeichendetektor, keine Korrektur.** Unter `CFD_FAC_APG_MOZ=1` ist κ ohne Physikwirkung und MUSS 1 sein (Wächter). |
| `CFD_FAC_APG_MOZ` (22.09.2026) | 0/1 (Default 0 = lineare Form, bitgleich). 1 = Mozaffari–Jacob–Sagaut 2024: tw = tw_Spalding·f², f = 1 − C/(1 + a0/α_p), α_p = ν·(dp/ds)/(ρ·u_τ³) aus dem GEKLEMMTEN Spalding-tw; FPG-Ast f ≡ 1 (Polstelle der Paperform bei α_p = −a0 vermieden); KEINE Klemme mehr. Braucht `CFD_FAC_APG=1`. Zähler [317] Entartung, [318..325] f-Histogramm, [326]/[327] Schatten der entfallenen Klemme; Abnahme im APG-Bericht (Σ[317..325] = [308], [318] < [308], [19]=[309]=[310]=0, [326]+[327] ≈ [316]). Gate-Arm `prod8nahM`. |
| `CFD_FAC_APG_C` / `CFD_FAC_APG_AP0` | Paperwerte 0,4 / 0,005 (Kalibrierkonstanten des Fits NACA-4412/Ahmed, keine Stellknöpfe). Nur mit MOZ=1 (sonst harter Fehler); 0 < C ≤ 1, a0 > 0, beide ≥ 5e-7 (6-Stellen-Emission). |
...
## Korrekturen 2026-08-20 (XL-Audit 3, Zusammenspiel/Doku)
