...
static void bericht_klassen(LBM_Domain* D, const std::vector<Facette>& F, const string& out_dir, const double tau_ziel, const string& ort) {
	...
	print_info("  links  y_w     n_fac    Besuche/fac   u_t        u_t_abt    y_abt   tw         twe        |P1|       s1         phi1       Rueckfall%   tw/Ziel  |P1|/twe");
	...
	fk << "eigene_links,yw,n_fac,besuche_je_fac,ut,ut_abtast,yw_abtast,tw,twe,absP1,s1,phi1,rueckfall_pct,tw_ziel,absP1_twe\n";
	uint zeilen=0u;
	for(const auto& e : V) { const Agg& g=e.second; const double v=g.v>0.0?g.v:1.0;
		const double ut=g.ut/v, tw=g.tw/v, twe=g.twe/v, p1=g.p1/v, s1=g.s1/v, phi=g.phi/v, rf=100.0*g.rf/v, tz=tau_ziel>0.0?tw/tau_ziel:0.0, pt=twe!=0.0?p1/fabs(twe):0.0, uta=g.uta/v, ywa=g.ywa/v;
		fk << e.first.first << "," << 0.01*e.first.second << "," << g.n << "," << g.v/(double)g.n << "," << ut << "," << uta << "," << ywa << "," << tw << "," << twe << "," << p1 << "," << s1 << "," << phi << "," << rf << "," << tz << "," << pt << "\n";
		if(zeilen++<16u) { char z[320]; snprintf(z, sizeof(z), "  %5u  %5.2f  %8lu  %11.1f  %9.5f  %9.5f  %6.2f  %9.3e  %9.3e  %9.3e  %+9.3e  %+9.3e  %8.1f    %6.3f   %7.2f", e.first.first, 0.01*e.first.second, (unsigned long)g.n, g.v/(double)g.n, ut, uta, ywa, tw, twe, p1, s1, phi, rf, tz, pt); print_info(string(z)); } }
...
void main_setup_kanal() {
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) { // ★ Stufe 2: Wirkpfad-Nachweis, Soll EXAKT (F7)
		...
		if(env_u("CFD_SGS_FDWAND",0u)>0u) { const ulong s39=(ulong)lbm.lbm_domain[0]->rho_clamp_hits[39]; if(s39==0ull) print_error("[Kanal] SGS_FDWAND war angefordert, aber Slot 39 = 0 -- stiller No-Op (Emission? Rebind? Enqueue?)."); else print_info("[Kanal] SGS_FDWAND-Wirkpfad Slot 39 = "+to_string(s39)+" (Soll ~ fac_N * ceil(n/100) abzueglich WANDFREI-Zellen)."); }
...
static void main_setup_fahrzeug_dd() {
	...
	ulong slot_alt[72]; for(uint i=0u;i<72u;i++) slot_alt[i]=0ull; bool slot_alt_da=false;
			...
			if(t_si>=ber_next) {
				...
				if(nb>0ull) {
					...
					{	LBM_Domain* d0 = lbm_f.lbm_domain[0];
						...
						for(uint i=0u;i<72u;i++) slot_alt[i]=(ulong)H[i]; slot_alt_da=true;
					}
				}
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) { // ★ Stufe 5: Pruefpfade IMMER (ausserhalb stat_ok -- Audit-R1-Muster)
		...
		if(env_u("CFD_SGS_FDWAND",0u)>0u) { const ulong s39=(ulong)df->rho_clamp_hits[39]; if(s39==0ull) print_error("[Nahfeld] SGS_FDWAND war angefordert, aber Slot 39 = 0 -- stiller No-Op."); else print_info("[Nahfeld] SGS_FDWAND-Wirkpfad Slot 39 = "+to_string(s39)+"."); }
		if(LBM_Domain::s_fac_elibb_pur) { // ★ Pruefbefund Messlogik-3 (2026-08-25): der Pur-Return sitzt VOR Slot 7 -- das alte Soll kannte den Pur-Modus nicht und print_error (=exit) toetete den projizierten Cd-Pfad. Pur-Soll: Slot 7 = 0, ELIBB-Wirkpfad (Slot 67) > 0.
			if(wz!=0ull) print_error("Pur-Arm: Slot 7 muesste 0 sein -- Return-Position verschoben?");
