...
static void pruefe_kaskade(const uint* H, const string& ort, const bool messnur, const bool pinv, const uint uw, const uint rdiag) { // ★ 06.09.: uw als PARAMETER, nicht als Static -- im dd-Fall wird LBM_Domain::s_fac_uw fuer das Fernfeld ueberschrieben (Pruefbefund B2)
	...
	{	const ulong rn=(ulong)H[143];
		if(rn>0ull) {
			...
			if((ulong)H[69]>0ull) print_error("["+ort+"] CFD_FAC_RDIAG: Slot 143 = 0, obwohl Slot 69 = "+to_string((ulong)H[69])+" Rueckfallbesuche zaehlt -- der RDIAG-Block wird nicht erreicht (Define nicht emittiert?) oder def_fac_tau*twe ist dort ueberall 0.");
...
static void main_setup_fahrzeug_dd() {
	...
	uint nahfeld_uw = 0u;
	const uint nahfeld_rdiag = LBM_Domain::s_fac_rdiag; // ★ B2 (07.09.): Konstruktionszeit-Kopie -- die Statik wird vor lbm_c wieder genullt, der Bericht laeuft danach // ★ 06.09. (Pruefbefund B2): u_w-Zustand der NAHFELD-Domaene. Das Static wird unten fuer lbm_c ueberschrieben; die Waechter am Laufende brauchen DIESEN Zustand, nicht den des Fernfelds.
