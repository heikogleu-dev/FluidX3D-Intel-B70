...
void main_setup_kugel() {
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) {
		...
		lbm.alloc_facetten(FF, elibb_an_kugel&&!elibb_qmap.empty()?&elibb_qmap:nullptr, 0u); if(env_u("CFD_SGS_GDIAG",0u)>0u) print_warning("CFD_SGS_GDIAG ist im KUGELFALL NICHT verdrahtet (kein Mess-Enqueue, kein Bericht -- Pruefbefund B-5). Schalter wird ignoriert; Verdrahtung bei Bedarf nachziehen.");
	}
