...
// Das ist dieselbe Groesse, nur in der Einheit, in der man sie denkt -- und sie macht den
// Zusammenhang sichtbar, den die Kommazahl verbirgt: die Laufzeit ist streng proportional zu N.
// CFD_U_LAT bleibt bestehen; beide zusammen sind ein Fehler, nicht eine Rangfolge.
static float u_lat_schalter(const char* wo) {
	const float vorgabe = U_LAT_VORGABE;
	const char* spz = getenv("CFD_SCHRITTE_PRO_ZELLE");
	if(spz!=nullptr&&getenv("CFD_U_LAT")!=nullptr) print_error("CFD_SCHRITTE_PRO_ZELLE und CFD_U_LAT sind BEIDE gesetzt. Sie bezeichnen dieselbe Groesse (u_lat = 1/Schritte je Zelle); welcher gilt, waere Auslegung. Genau einen setzen.");
	float u = env_f("CFD_U_LAT", vorgabe);
	if(spz!=nullptr) {
		const double n_spz = atof(spz);
		if(!(n_spz>0.0)) print_error("CFD_SCHRITTE_PRO_ZELLE = "+string(spz)+" ist nicht positiv. Der Wert ist die Zahl der Zeitschritte, die die Anstroemung fuer eine Zelle braucht -- u_lat = 1/N.");
		u = (float)(1.0/n_spz);
	}
	...
	} else print_info("Gittergeschwindigkeit u_lat = "+to_string(u,5u)+" = "+to_string(1.0f/u,3u)+" Schritte je Zelle (Vorgabe; weder CFD_SCHRITTE_PRO_ZELLE noch CFD_U_LAT abweichend), Ma = "+to_string(ma_soll,4u));
	// ★ 12.09.2026: DIE SCHRITTSKALIERUNG SETZEN -- hier und nur hier, vor jedem Leser.
	ulat_skal_setzen((double)vorgabe/(double)u);
...
// ★ 12.09.2026 (Heiko): DIE GEMEINSAME LESEFUNKTION FUER SCHRITT-SCHALTER.
// Jeder Schalter, der in ZEITSCHRITTEN zaehlt, wird ab jetzt hierdurch gelesen -- nicht mehr mit
// env_u direkt. Sie tut drei Dinge: den Wert (oder seinen Code-Default) holen, ihn mit
// ulat_skal() auf dieselbe PHYSIKALISCHE Zeit umrechnen, und die Umrechnung einmal je Name
// ansagen. Der Code-Default wird MITSKALIERT, und das ist der Punkt: er ist fuer u_lat = 0,075
// gewaehlt, ein ungesetzter Schalter trug die Verschiebung also genauso wie ein gesetzter --
// nur unsichtbar, weil der alte Waechter nur GESETZTE Namen sah.
// NICHT hierdurch gehen Schalter, die in ABTASTUNGEN oder Vielfachen zaehlen (CFD_FAC_CD_EVERY
// ist ein Vielfaches der Abtastkadenz und damit dimensionslos) -- die wuerden sonst doppelt
...
static uint env_schritte(const char* name, const uint vorgabe) {
	...
	// diese Mechanik verhindern soll, nur mit umgekehrtem Vorzeichen.
	if(roh==0u||sk==1.0) return roh;
	const long long neu_roh = llround((double)roh*sk);
	const uint neu = (uint)(neu_roh<1ll ? 1ll : neu_roh); // 0 bricht anderswo ab (alpha = 1/T)
...
static void u_lat_zeitwaechter(const float u_lat, const float dt_f, const float dt_c,
	...
	for(uint i=0u; i<n_liste; i++) {
		...
		// ★ 12.09.2026 UMGESCHRIEBEN: bis heute empfahl diese Meldung eine Umrechnung VON HAND und
		// konnte nicht wissen, ob der Wert schon umgerechnet war. Seit env_schritte ist die Frage
...
static void pruefe_masse_alle(const uint* H, const bool an, const bool messnur, const uint kraft, const string& ort, const bool x=false, const uint uw=0u) { // ★ 06.09.: uw (Pruefbefund M4)
	...
	if(an&&s92==0ull) print_error("["+ort+"] CFD_FAC_MASSE_ALLE angefordert, aber Slot 92 = 0 -- die Kompensation ueber alle 19 Links wird NIE erreicht (stiller No-Op: Emission? pass2_an?).");
		...
		print_info("   MASSE_ALLE-Wirkpfad Slot 92 = "+to_string(s92)+" angewandte Besuche mit neu verteilter Kompensation (Masse zellweise erhalten, Kompensation impulsfrei).");
		// Modus 2 legt beta3 komplett auf f_0 -- die Stoerung sitzt auf EINER Population, relativ dreifach.
		// Slot 93 zaehlt Besuche, bei denen f_0 danach nicht mehr positiv ist. Soll: 0. Der Zaehler ist
		// nur unter FACETTEN_MASSE_F0 im Kernel; in Modus 1 ist er konstruktiv 0.
		const ulong s93=(ulong)H[93];
		if(s93>0ull) print_warning("   MASSE_ALLE Modus 2: Slot 93 = "+to_string(s93)+" Besuche mit f_0 <= 0 nach der Kompensation ("+to_string((float)(100.0*(double)s93/(double)(s92>0ull?s92:1ull)),4u)+" % des Wirkpfads). Die Ruhepopulation kippt dort ins Negative -- Positivitaet ist verletzt.");
		else print_info("   Positivitaetsprobe f_0 (Slot 93) = 0 -- keine negative Ruhepopulation.");
		if(x) { // ★ ARM X: Kreuztabelle roh x Schatten. 69 = roh_RF + 94 (Buchung); roh_RF = beide_RF + 95
			const ulong s94=(ulong)H[94], s95=(ulong)H[95], s69=(ulong)H[69], wp=(ulong)H[7], s9=(ulong)H[9], s17=(ulong)H[17];
			const ulong erreicht = wp>(s9+s17) ? wp-s9-s17 : 0ull;
			const ulong roh_rf = s69>s94 ? s69-s94 : 0ull, beide_rf = roh_rf>s95 ? roh_rf-s95 : 0ull, beide_an = erreicht>s69 ? erreicht-s69 : 0ull;
			const double n = (double)(erreicht>0ull?erreicht:1ull);
			const double q_x = 100.0*(double)beide_an/n, q_roh = 100.0*(double)(erreicht>roh_rf?erreicht-roh_rf:0ull)/n, q_a2 = 100.0*(double)(erreicht>(beide_rf+s94)?erreicht-beide_rf-s94:0ull)/n;
			print_info("["+ort+"] ARM X KREUZTABELLE (t%100, Nenner Wirkpfad-Skips = "+to_string(erreicht)+"):");
...
static void pruefe_kaskade(const uint* H, const string& ort, const bool messnur, const bool pinv, const uint uw, const uint rdiag) { // ★ 06.09.: uw als PARAMETER, nicht als Static -- im dd-Fall wird LBM_Domain::s_fac_uw fuer das Fernfeld ueberschrieben (Pruefbefund B2)
	...
	// ★★ 07.09.2026 RUECKFALL-DIAGNOSE (CFD_FAC_RDIAG, Slots 136..149). Beantwortet K1: ist das
	// phi1 der zurueckgefallenen Facetten ein REIBUNGSdefekt oder fehlgebuchter Wanddruck?
	{	const ulong rn=(ulong)H[143];
		if(rn>0ull) {
			ulong rh=0ull; for(uint b=0u; b<5u; b++) rh += (ulong)H[136u+b];
			const ulong rp=(ulong)H[141], rm=(ulong)H[142];
			// ★ B7: innere Abnahme -- konstruktiv gilt Summe(136..140) == [143].
			if(rh!=rn) print_warning("["+ort+"] RDIAG: Summe der Druckrest-Eimer "+to_string(rh)+" != Nenner "+to_string(rn)+" -- die Eimerkette ist nicht dicht (Wickel oder fehlender Zweig).");
			string rz=""; const char* gr[5]={"<0,001","0,001-0,01","0,01-0,1","0,1-1",">=1"};
			for(uint b=0u; b<5u; b++) rz += (b>0u?" | ":"")+string(gr[b])+": "+to_string((float)(100.0*(double)H[136u+b]/(double)rh),1u)+" %";
			print_info("["+ort+"] RUECKFALL-DIAGNOSE ueber "+to_string(rn)+" gesampelte Rueckfallbesuche mit Ziel (Slots 136..154, NICHT hash-ausgeduennt):");
			print_info("   Rest-Druckterm |2(rho-1)(S1.t1)| / |Ziel| an RUECKFALLfacetten: "+rz);
			// ★ B6: a_rf ist BITGENAU 0, wo S1.t1 == 0 (achsparallel mit symmetrischer Linkmenge) oder rho == 1
			//   exakt. Diese Besuche zaehlen in KEINEN der beiden Vorzeichen-Slots, stehen aber im Nenner --
			//   ohne den Rest gelesen, sieht ein solcher Lauf aus wie "hebt sich weg".
			const double p_p=100.0*(double)rp/(double)rn, p_m=100.0*(double)rm/(double)rn;
			print_info("   dessen Vorzeichen: + "+to_string((float)p_p,1u)+" % | - "+to_string((float)p_m,1u)+" % | exakt 0: "+to_string((float)(100.0-p_p-p_m),1u)+" %");
			print_info("   LESART: + und - nahe gleichauf = der Term hebt sich ueber die Treppenperiode weg; einseitig = er steht systematisch. Ein hoher Anteil 'exakt 0' heisst NICHT 'hebt sich weg', sondern dass S1.t1 oder (rho-1) dort bitgenau null ist.");
			// ★ B4: zwei GETRENNTE Eimersaetze. Nur [144..148] ist eine Messung.
			ulong sg=0ull, sr=0ull; for(uint b=0u; b<5u; b++) { sg += (ulong)H[144u+b]; sr += (ulong)H[150u+b]; }
			if(sg+sr+(ulong)H[149]!=rn) print_warning("["+ort+"] RDIAG: s1_soll-Eimer ("+to_string(sg)+"+"+to_string(sr)+") + G11roh~0 ("+to_string((ulong)H[149])+") != Nenner "+to_string(rn)+" -- Kette nicht dicht.");
			const char* gs[5]={"<0","0-0,1","0,1-0,5","0,5-1,0",">=1,0"};
			if(sg>0ull) { string sz=""; for(uint b=0u; b<5u; b++) sz += (b>0u?" | ":"")+string(gs[b])+": "+to_string((float)(100.0*(double)H[144u+b]/(double)sg),1u)+" %";
				print_info("   s1_soll/u_t an GATE-Rueckfaellen (G11 > 0, "+to_string(sg)+" Besuche -- der Loeser hat gerechnet, ein Gate hat verworfen): "+sz);
				print_info("   ACHTUNG (B5): Slot-10-Rueckfaelle feuern per Definition bei |s1| > 2*budget*u_t und landen deshalb KONSTRUKTIV im obersten Eimer. Ein voller Eimer >=1,0 belegt zunaechst nur, dass das Budget-Gate gearbeitet hat."); }
			if(sr>0ull) { string sz=""; for(uint b=0u; b<5u; b++) sz += (b>0u?" | ":"")+string(gs[b])+": "+to_string((float)(100.0*(double)H[150u+b]/(double)sr),1u)+" %";
				print_info("   dasselbe an RANG-0-Rueckfaellen (G11 == 0, Einzellink, "+to_string(sr)+" Besuche): "+sz);
				print_info("   DAS IST KEIN SOLLWERT (B4): bei Rang 0 ist der unter der Massen-Nebenbedingung erreichbare Unterraum LEER (q_i == 0 fuer JEDES u_s, kernel.cpp:2151-2163). R1/G11roh beantwortet nur, was OHNE diese Nebenbedingung noetig waere -- genau der am 25.08. verworfene Weg. Als Groessenordnung lesbar, nicht als Ziel."); }
			if((ulong)H[149]>0ull) print_info("   "+to_string((ulong)H[149])+" Besuche mit G11roh ~ 0 (c_1 parallel n, kipp45-Klasse): dort gibt es weder tangentiale BB-Bremse noch Injektionsmoeglichkeit, s1_soll ist undefiniert.");
		} else if(rdiag>0u) {
			// ★ B1: print_error beendet den Prozess (utilities.hpp) und haette den GESAMTEN restlichen
			//   Endbericht gekostet -- Kaskaden-Abnahme, u_w-Abnahme, bericht_klassen samt
			//   facetten_klassen.csv. Ein Lauf ganz ohne Rueckfallbesuche ist aber legitim. Slot 69
			//   (rueckfall, ungegatet durch das Ziel) unterscheidet die beiden Faelle.
			// ★ E-5 (Auditor E, 07.09.): HIER DARF KEIN print_error STEHEN. pruefe_kaskade laeuft im
			//   Nahfeld-Block VOR bericht_klassen (setup.cpp ~7940/7941); print_error ist exit(1)
			//   (utilities.hpp). Ein RDIAG-Fehlbefund nach 1,5 h Laufzeit haette facetten_klassen.csv
			//   und yplus_facetten.csv mitgenommen -- also genau die Tabelle, fuer die RDIAG gebaut
			//   wurde. Ein DIAGNOSE-Defekt rechtfertigt keine Vernichtung der Messung. Die Iron-Rule-
			//   Sichtbarkeit ("Schalter ohne feuernden Zaehler ist ein harter Fehler") bleibt gewahrt:
			//   die Queue zaehlt Warnungen je Lauf und schreibt sie in die ENDE-Zeile.
			if((ulong)H[69]>0ull) print_warning("["+ort+"] CFD_FAC_RDIAG IST EIN NO-OP: Slot 143 = 0, obwohl Slot 69 = "+to_string((ulong)H[69])+" Rueckfallbesuche zaehlt. Entweder wird der Block nicht erreicht (Define nicht emittiert) oder def_fac_tau*twe ist dort ueberall 0. Die RDIAG-Zeilen dieses Laufs sind wertlos, alles Uebrige bleibt gueltig.");
			else print_info("["+ort+"] CFD_FAC_RDIAG aktiv, aber kein einziger Rueckfallbesuch (Slot 69 = 0) -- in diesem Arm loesen alle Facetten, es gibt nichts zu diagnostizieren.");
		}
	}
	if(uw>0u) {
...
// ★★ 08.09.2026 VAN DRIEST auf der Facetten-Architektur -- Host-Seite: Selbsttest und Bericht.
// Kernel: kernel.cpp im FDWAND-Zweig von stream_collide unter #ifdef SGS_VANDRIEST.
// Die Daempfung D = 1-exp(-y+/A+) wird mit dem y+ des WANDMODELLS gebildet (tw-Laufmittel aus der
// Spalding-Kette, y_w aus der Voxelgeometrie) -- nicht mit einem Strain-Schaetzer wie in V1, wo genau
// diese Selbstreferenz an der Abloesung nu_t auf 0 trieb.
static void sgs_vandriest_selbsttest() {
	static bool einmal=false; if(einmal) return; einmal=true;
	// (1) Grenzwerte: y+ -> 0 gibt D -> 0 (volle Daempfung), y+ -> gross gibt D -> 1 (inert).
	const double A=26.0;
	const double d0 = 1.0-exp(-0.0/A), dinf = 1.0-exp(-1000.0/A);
	if(fabs(d0)>1e-12||fabs(dinf-1.0)>1e-9) print_error("VAN-DRIEST-Selbsttest VERLETZT (1): Grenzwerte falsch, D(0) = "+to_string((float)d0,8u)+", D(1000) = "+to_string((float)dinf,8u)+" (Soll 0 und 1).");
	// (2) Literaturpunkte gegen von Hand gerechnete Werte (van Driest 1956, A+ = 26).
	const double yp[4]={5.0,26.0,50.0,100.0}, soll[4]={0.17494703,0.63212056,0.85384344,0.97863826}; // exakt gerechnet 08.09. (erste Fassung hatte drei von vier Werten aus dem Kopf -- der Test hat es gefangen)
	for(int i=0;i<4;i++) {
		const double d=1.0-exp(-yp[i]/A);
		if(fabs(d-soll[i])>1e-6) print_error("VAN-DRIEST-Selbsttest VERLETZT (2): D(y+ = "+to_string((float)yp[i],1u)+") = "+to_string((float)d,8u)+" gegen Referenz "+to_string((float)soll[i],8u)+".");
	}
	// (3) entfaellt (Pruefagent 08.09., Befund 7): eine double-Rechnung gegen sich selbst prueft die EMITTIERTE Konstante
	// nicht. Die y+-Skalierung 2*def_fac_Y sieht nur der Ist=Soll in bericht_vandriest -- Geraete-Histogramm gegen
	// Host-y+ aus get_nu() -- und der laeuft in jedem Messarm.
	// (4) Monotonie und Quadrat: D^2 muss in y+ streng wachsen und in [0,1] liegen.
	double vor=-1.0;
	for(double y=0.1;y<400.0;y*=1.3) { const double d2=pow(1.0-exp(-y/A),2.0);
		if(!(d2>vor)||d2<0.0||d2>1.0) print_error("VAN-DRIEST-Selbsttest VERLETZT (4): D^2 nicht streng wachsend oder ausserhalb [0,1] bei y+ = "+to_string((float)y,3u)+".");
		vor=d2; }
	print_info("VAN-DRIEST-Selbsttest bestanden: Grenzwerte, vier Stuetzpunkte auf 1e-6 (y+ = 5/26/50/100 -> D = 0,175/0,632/0,854/0,979), D^2 streng wachsend in [0,1]. Die Emission 2*def_fac_Y prueft erst der Ist=Soll im Lauf.");
}
// Bericht: Wirkpfad, D^2-Histogramm vom GERAET und der Abgleich gegen das Host-y+ desselben Laufs.
// Der Abgleich ist die eigentliche Abnahme des Messarms: trifft das geraetseitige D^2 nicht die
...
static void bericht_vandriest(LBM_Domain* D, const float aplus, const string& ort) {
	...
	// y+ = sqrt(tw)*fac_geo[8i+3]/nu. So ist der Vergleich in jedem Fall derselbe Ausdruck --
	// der Kanal-y+ weiter unten stammt aus dem Kraftfeld und taugt dafuer NICHT.
	std::vector<float> yplus_host; yplus_host.reserve(D->fac_N);
	{	const float nu_h = D->get_nu();
		for(ulong i=0ull;i<D->fac_N;i++) if(D->fac_tau_n[i]>0u) {
			const float tw=D->fac_tau[6ull*i]/(float)D->fac_tau_n[i];
			yplus_host.push_back(sqrt(fmax(0.0f,tw))*D->fac_geo[8ull*i+3ull]/nu_h);
		} }
	const ulong wp=(ulong)H[168], ohne=(ulong)H[169];
	...
	if(ges!=wp) print_error("["+ort+"] VAN DRIEST: Summe der Eimer 160..167 = "+to_string(ges)+" != Wirkpfad 168 = "+to_string(wp)+" -- beide stehen hinter demselben Gatter und MUESSEN gleich sein (Saettigung 0xF0000000? fremder Schreiber?).");
	string hist; double d2m=0.0;
	for(uint b=0u;b<8u;b++) { const double a=(double)H[160u+b]/(double)(ges>0ull?ges:1ull);
		d2m += a*((double)b+0.5)/8.0; hist += (b?" | ":"")+to_string((float)(100.0*a),1u)+" %"; }
	print_info("["+ort+"] VAN DRIEST "+string(modus>1u?"ANGEWANDT":"MESSARM")+": Wirkpfad Slot 168 = "+to_string(wp)
		+", Facetten ohne Besuch (169) = "+to_string(ohne)+"; D^2-Histogramm ueber ALLE Zaehlslots ab der Sperre (Zeitintegral, 8 Eimer): "+hist
		+"; <D^2> aus Eimermitten ~ "+to_string((float)d2m,4u)+" (Verlaufsdiagnostik, KEIN Ist=Soll -- das Laufmittel wandert zwischen Sperre und Laufende).");
	...
	if(!yplus_host.empty()&&gl>0ull) {
		ulong hh[8]={0,0,0,0,0,0,0,0}; double hm=0.0;
		for(float y : yplus_host) { const double d=1.0-exp(-(double)y/(double)aplus); const double d2=d*d; hm+=d2;
			uint bb=(uint)(d2*8.0); if(bb>7u) bb=7u; hh[bb]++; }
		hm/=(double)yplus_host.size();
		double maxpp=0.0, maxpp_int=0.0; string hh_s, hl_s;
		for(uint b=0u;b<8u;b++) { const double ah=(double)hh[b]/(double)yplus_host.size(), al=(double)H[170u+8u*bank+b]/(double)gl, ai=(double)H[160u+b]/(double)(ges>0ull?ges:1ull);
			maxpp=fmax(maxpp, fabs(ah-al)); maxpp_int=fmax(maxpp_int, fabs(ah-ai));
			hh_s += (b?" | ":"")+to_string((float)(100.0*ah),1u)+" %"; hl_s += (b?" | ":"")+to_string((float)(100.0*al),1u)+" %"; }
		print_info("["+ort+"] VAN DRIEST Letzt-Stichprobe Slot "+to_string(L)+" ("+to_string(gl)+" Facettenzellen gegen "+to_string((ulong)yplus_host.size())+" Host-Facetten): Geraet "+hl_s+" || Host "+hh_s+". Das Zeitintegral weicht max. "+to_string((float)(100.0*maxpp_int),1u)+" Prozentpunkte vom Host ab (nur Information).");
		if(maxpp>0.02) print_error("["+ort+"] VAN DRIEST Ist=Soll VERLETZT: Letzt-Stichprobe weicht in einem Eimer um "+to_string((float)(100.0*maxpp),2u)+" Prozentpunkte vom Host-Endzustand ab (Schranke 2, hoechstens 99 Schritte Abstand). Der Kernel rechnet mit einem ANDEREN y+ als der Report -- alles Weitere waere wertlos.");
		else print_info("["+ort+"] VAN DRIEST Ist=Soll: Letzt-Stichprobe trifft den Host-Endzustand in allen 8 Eimern (max. "+to_string((float)(100.0*maxpp),2u)+" Prozentpunkte, Schranke 2). Host-<D^2> exakt = "+to_string((float)hm,4u)+" (mittlere nu_t-Senkung "+to_string((float)(100.0*(1.0-hm)),1u)+" %).");
	}
	// KONTAMINATIONSMASS (Pruefagent 08.09., Befund 3): das Modell-y+ stammt aus dem Akkumulator ueber ALLE Besuche, auch
	// Rueckfallbesuche mit "haette"-tw. Liegt KDIAG an, steht daneben das y+ NUR ueber angewandte Besuche (fac_kd[16i+10]/[16i+11]);
	// die Differenz der D^2-Verteilungen ist der Preis dieser Wahl -- in Prozentpunkten statt als Vermutung.
	if(D->fac_kdiag_on&&D->fac_kd.length()>=16ull*D->fac_N&&!yplus_host.empty()) { D->fac_kd.read_from_device();
		ulong ha[8]={0,0,0,0,0,0,0,0}, hk[8]={0,0,0,0,0,0,0,0}, na=0ull; const float nu_h=D->get_nu();
		for(ulong i=0ull;i<D->fac_N;i++) { const float va=D->fac_kd[16ull*i+11ull]; if(va<=0.0f) continue;
			const double ya=(double)(sqrt(fmax(0.0f,D->fac_kd[16ull*i+10ull]/va))*D->fac_geo[8ull*i+3ull]/nu_h), d=1.0-exp(-ya/(double)aplus);
			uint bb=(uint)(d*d*8.0); if(bb>7u) bb=7u; ha[bb]++; na++; }
		for(float y : yplus_host) { const double d=1.0-exp(-(double)y/(double)aplus); uint bb=(uint)(d*d*8.0); if(bb>7u) bb=7u; hk[bb]++; }
		double maxk=0.0; string ha_s;
		for(uint b=0u;b<8u;b++) { const double aa=na>0ull?(double)ha[b]/(double)na:0.0, ak=(double)hk[b]/(double)yplus_host.size(); maxk=fmax(maxk,fabs(aa-ak)); ha_s += (b?" | ":"")+to_string((float)(100.0*aa),1u)+" %"; }
		print_info("["+ort+"] VAN DRIEST Kontaminationsmass: D^2-Verteilung aus y+ NUR ueber ANGEWANDTE Besuche ("+to_string(na)+" Facetten): "+ha_s+" -- weicht max. "+to_string((float)(100.0*maxk),1u)+" Prozentpunkte von der Modell-Eingangsreihe (alle Besuche) ab. Das Modell rechnet DEKLARIERT mit der Reihe ueber alle Besuche.");
	}
}
static void sgs_sism_selbsttest() {
	static bool einmal=false; if(einmal) return; einmal=true;
	// (1) Klemme und Abzugsformel gegen eine double-Referenz: w = 1/(tau0 + 3*c2*max(0, |S| - Sbar)).
	const float w0 = 1.9f, c2 = 0.030021f; const float tau0 = 1.0f/w0;
	const float w_klemme = 1.0f/(tau0+3.0f*c2*fmax(0.0f, 0.010f-0.020f)); // |S| < Sbar -> nu_t = 0 -> molekulares w
	if(w_klemme!=1.0f/tau0) print_error("SISM-Selbsttest VERLETZT (1a): die Klemme liefert nicht bitgenau das molekulare w ("+to_string(w_klemme,8u)+" gegen "+to_string(1.0f/tau0,8u)+").");
	const double w_ref = 1.0/((double)tau0+3.0*(double)c2*(0.030-0.010));
	const float w_ist = 1.0f/(tau0+3.0f*c2*fmax(0.0f, 0.030f-0.010f));
	if(fabs((double)w_ist-w_ref)>1e-6*w_ref) print_error("SISM-Selbsttest VERLETZT (1b): Abzugsformel "+to_string(w_ist,8u)+" gegen double-Referenz "+to_string((float)w_ref,8u)+".");
	// (2) EMA-Rekursion sb += a*(S - sb) in float mit fma (wie im Kernel) gegen die geschlossene Form 1-(1-a)^n, T = 100.
	const uint T=100u; const float a=1.0f/(float)T; float sb=0.0f;
	for(uint n=1u; n<=10u*T; n++) { sb = fma(a, 1.0f-sb, sb);
		if(n==3u*T||n==10u*T) { const double soll=1.0-pow(1.0-(double)a,(double)n);
			if(fabs((double)sb-soll)>2e-6) print_error("SISM-Selbsttest VERLETZT (2): EMA nach "+to_string(n)+" Schritten "+to_string(sb,7u)+" gegen geschlossene Form "+to_string((float)soll,7u)+"."); } }
	// (3) Jensen: |<S>| < <|S|> fuer einen oszillierenden Tensor -- genau der Unterschied zwischen der gebauten
	//     Tensor-EMA und der verworfenen Form <|S|> (die im Zeitmittel nu_t = 0 liefert).
	float sbx=0.0f, sby=0.0f; double sum_abs=0.0; const uint n_=4000u;
...
// unter |S| an jeder Facette), 100 % = nu_t identisch 0 = WANDFREI-Zustand (07.09. nach 175 Schritten divergiert),
// dazwischen Physik (Fluktuationen unter dem Mittel). Liegt die Sperre hinter dem Laufende (ab >= t), ist Slot 126 = 0
// KORREKT -- der Arm ist dann der bitgleiche FDWAND-Bezug, kein No-Op.
// ★★ 08.09.2026 SGS-BAND: Wirkpfad-Abnahme. Iron Rule "jeder Mechanismus beweist seine Wirkung im
// Binary" -- Slot 186 zaehlt jede behandelte Bandzelle an den t%100-Stichproben. Soll ist exakt
// band_N x Zahl der Zaehlslots; jede Abweichung heisst, dass Zellen den Leserzweig nicht erreichen
// (Maske und Liste auseinander) oder dass fremde Zellen ihn erreichen (Mengen nicht disjunkt).
static void pruefe_band_wirkpfad(const LBM_Domain* D, const ulong t_ende, const string& ort) {
	...
	const double abw = fabs((double)wp-(double)soll)/(double)soll;
	if(abw>0.02) print_error("["+ort+"] SGS-BAND Wirkpfad Slot 186 = "+to_string(wp)+", Soll "+to_string(soll)+" = "+to_string(D->band_N)+" Bandzellen x "+to_string(slots)
...
// soll und ein stiller No-Op die Antwort still ins Gegenteil drehen wuerde:
//   1. Slot 188 == Slot 76. Der Skalierer sitzt IM FDWAND-Zweig und wird von jeder Facettenzelle
//      durchlaufen, die auch FDWAND anwendet. Weichen sie ab, steht der Block an der falschen Stelle.
//   2. Slot 190 > 0. w hat sich wirklich geaendert -- gegen den Fall, dass der Faktor emittiert, aber
//      nicht wirksam wurde (die Lehre aus dem Moving-Floor-No-Op und aus HRR/MOZAFFARI_APG in V1,
//      wo ein Define host-seitig stand und nie an OpenCL ging).
//   3. Summe der Histogrammfaecher 191..198 == Slot 188. Faellt sie auseinander, zaehlt der Gate anders
//      als der Histogrammzweig und die Verteilung ist nicht lesbar.
static void pruefe_nut_skal_wirkpfad(const uint* H, const float skal, const string& ort) {
	if(skal==1.0f) return;
	const ulong wp=(ulong)H[188], nut_pos=(ulong)H[189], geaendert=(ulong)H[190], fdwand=(ulong)H[76];
	ulong hist=0ull; for(uint b=0u; b<8u; b++) hist += (ulong)H[191u+b];
	if(wp==0ull) { print_error("["+ort+"] CFD_SGS_NUT_SKAL = "+to_string(skal,4u)+" angefordert, aber Slot 188 = 0 -- STILLER NO-OP. Emission (SGS_NUT_SKAL/def_sgs_nut_skal)? Statik vor device_defines gesetzt? WANDFREI davor?"); return; }
	if(wp!=fdwand) print_error("["+ort+"] NUT_SKAL: Slot 188 = "+to_string(wp)+" ungleich Slot 76 (FDWAND angewandt) = "+to_string(fdwand)+". Der Skalierzweig wird NICHT von genau den Zellen durchlaufen, die FDWAND anwenden -- er steht an der falschen Stelle im Block.");
	else print_info("["+ort+"] NUT_SKAL Ist=Soll: Slot 188 = Slot 76 = "+to_string(wp)+" -- der Skalierer erreicht die Facettenzellen vollstaendig und ausschliesslich.");
	if(geaendert==0ull) print_error("["+ort+"] NUT_SKAL: Slot 190 = 0 -- w hat sich an KEINER Zelle geaendert, obwohl der Faktor "+to_string(skal,4u)+" ungleich 1 ist. Der Arm ist wirkungslos und seine Kraftzahlen beweisen nichts.");
	if(nut_pos==0ull) print_error("["+ort+"] NUT_SKAL: Slot 189 = 0 -- nu_t war an KEINER Facettenzelle groesser als null, es gab also nichts zu skalieren. Kommt das w wirklich aus dem FD-Kernel?");
	if(hist!=wp) print_error("["+ort+"] NUT_SKAL: Histogrammsumme 191..198 = "+to_string(hist)+" ungleich Slot 188 = "+to_string(wp)+" -- Gate und Histogrammzweig zaehlen verschieden (oder Slot 188 ist gesaettigt); die Verteilung ist nicht lesbar.");
	else {
		const double N=(double)(hist>0ull?hist:1ull);
		print_info("["+ort+"] NUT_SKAL nu_t/nu_mol VOR der Skalierung (Anteile in %): =0: "+to_string((float)(100.0*(double)H[191]/N),2u)
			+" | <1: "+to_string((float)(100.0*(double)H[192]/N),2u)+" | <10: "+to_string((float)(100.0*(double)H[193]/N),2u)
			+" | <30: "+to_string((float)(100.0*(double)H[194]/N),2u)+" | <100: "+to_string((float)(100.0*(double)H[195]/N),2u)
			+" | <300: "+to_string((float)(100.0*(double)H[196]/N),2u)+" | <1000: "+to_string((float)(100.0*(double)H[197]/N),2u)
			+" | >=1000: "+to_string((float)(100.0*(double)H[198]/N),2u)
			+". Das ist die Daempfung, die der Faktor "+to_string(skal,4u)+" herunterskaliert -- an Wandzellen liegt der Median der Feldnachrechnung vom 10.09. bei 28 (4 mm) bzw. 59 (8 mm).");
	}
	print_info("["+ort+"] NUT_SKAL-Wirkpfad: Slot 188 (besucht) = "+to_string(wp)+", 189 (nu_t > 0) = "+to_string(nut_pos)+", 190 (w geaendert) = "+to_string(geaendert)+", Faktor "+to_string(skal,4u)+". DEKLARIERTER MESSARM, keine Produktionseinstellung -- der Faktor ist aus dem SISM-Lauf abgelesen (Lage 1, 4 mm: 1 - 0,852 = 0,148).");
}
static void pruefe_sism_wirkpfad(const uint* H, const ulong t_ende, const ulong ab, const uint T, const bool sism_on, const string& ort) {
	if(!sism_on) { print_error("["+ort+"] CFD_SGS_SISM angefordert, aber Konstruktionszustand sism_on = aus -- Statik-Lebensdauer-Falle (Emission ohne SGS_SISM), stiller No-Op."); return; }
	const ulong s126=(ulong)H[126], s127=(ulong)H[127];
	if(ab>=t_ende) {
		if(s126!=0ull||s127!=0ull) print_error("["+ort+"] SISM: Sperre ab = "+to_string(ab)+" liegt hinter dem Laufende t = "+to_string(t_ende)+", trotzdem Slot 126/127 = "+to_string(s126)+"/"+to_string(s127)+" -- der Phase-2-Zweig lief, obwohl er nicht durfte.");
	...
	if(s126>=0xF0000000ull) print_warning("["+ort+"] SISM: Slot 126 gesaettigt -- der Klemmanteil ist nicht belastbar.");
	const double anteil = 100.0*(double)s127/(double)s126;
	if(s127>=s126) print_warning("["+ort+"] SISM: Klemme greift an 100 % der Stichproben (127 = "+to_string(s127)+", 126 = "+to_string(s126)+") -- nu_t identisch 0 an allen Facetten = WANDFREI-Zustand (07.09. divergiert). Sperre ab zu kurz gegen T (Anfahrtransient in <S>)? Als WARNUNG, nicht exit: das ist ein Physik-Ergebnis, und genau dann braucht man den Rest des Endberichts (Pruefbefund 3).");
	if(s127==0ull) print_warning("["+ort+"] SISM: Klemme (Slot 127) hat NIE gegriffen -- Sbar bleibt an jeder Facette unter |S|: EMA tot oder T viel zu gross gegen die Laufzeit? sism_sbar.csv pruefen.");
	print_info("["+ort+"] SISM-Wirkpfad: Slot 126 (Abzug aktiv) = "+to_string(s126)+", Slot 127 (Klemme |S|<Sbar) = "+to_string(s127)+" = "+to_string((float)anteil,1u)+" % (Phase 2 ab Schritt "+to_string(ab)+", t%100-Stichprobe).");
}
// Sbar-Zeitreihe (Iron Rule: Diagnostik gehoert in den Code -- Zwischenergebnis, nicht nur Endzahl). Liest fac_sb
...
static void bericht_gdiag(LBM_Domain* D, const std::vector<Facette>& F, const string& out_dir, const string& ort) {
	...
	// ★ B3 (Pruefagent, 07.09.): NULLBESUCHS-WAECHTER. Iron Rule: ein Schalter ohne feuernden
	//   Zaehler ist ein harter Fehler. Genau hier ist am 07.09. ein Lauf durchgeschluepft
	//   (kd_kugel_gdiag2): alloziert, Bericht gedruckt, Selbsttest bestanden -- und in JEDER
	//   Zeile der CSV besuche_je_fac = 0, weil der Mess-Enqueue in der Kugel-Zeitschleife fehlte.
	//   Nichts hat widersprochen. Der Waechter prueft die Besuchssumme, bevor gemittelt wird.
	{	double bes=0.0; ulong kk=0ull;
		for(const Facette& f : F) { if(f.klasse!=0u) continue; if(8ull*kk+7ull>=D->fac_gd.length()) break; bes += (double)D->fac_gd[8ull*kk+5ull]; kk++; }
		if(kk>0ull&&bes<=0.0) print_error("["+ort+"] g-DIAGNOSE: "+to_string(kk)+" Wandzellen alloziert und Bericht erreicht, aber die Besuchssumme ist 0 -- der Mess-Enqueue laeuft nicht (sgs_gdiag_gpu() fehlt in der Zeitschleife dieses Falls). Jede Zahl aus diesem Lauf waere eine Null, die wie eine Messung aussieht.");
	}
...
static void bericht_zielerfuellung(const uint* H, const ulong wp, const ulong s9, const ulong s17, const ulong s69, const string& ort, const uint uw) {
	...
	if(wp>=0xF0000000ull||s69>=0xF0000000ull) print_info("   GEGENPROBE nicht pruefbar (Slot 7 gewickelt / 69 gesaettigt) (S9).");
	else if(fabs(q_hist-q_slot)>2.0) print_warning("   Die beiden Instrumente weichen um "+to_string((float)fabs(q_hist-q_slot),2u)
	...
	if(ohne>0ull) print_info(string("     ohne Ziel  (")+(env_u("CFD_FACETTEN",0u)==4u?"Nullziel-Arm def_fac_tau = 0":"twe = 0")+")   "+to_string(ohne)+"  ("+to_string((float)(100.0*(double)ohne/da),2u)+" % -- kein Wandschub gefordert, r undefiniert)");
...
static void schreibe_vtk_feld(LBM& L, const uint Nx, const uint Ny, const uint Nz,
	...
	f << "\nSCALARS rho float 1\nLOOKUP_TABLE default\n";
	{
		...
		for(uint z=0u; z<Sz; z++) for(uint y=0u; y<Sy; y++) {
			if(rr_&&y==0u) L.rho_schicht_in_host(z*stride, false); // unter RHO_RAND holt der Writer rho selbst je Schicht (kein Voll-Read)
...
struct KlemmBilanz {
	bool init = false, aus_gemeldet = false; uint alt[LBM_Domain::hits_n]; double ges[LBM_Domain::hits_n], nach[LBM_Domain::hits_n]; // ★ P1a: 288 -> hits_n (320)
	...
	std::ofstream csv;
	std::ofstream csv_pos; uint pos_modus = 0u, pos_haken = 0u, schale_modus = 0u; // ★ Audit 16.09.2026 (C-M2/A-N2): pos_haken ist die Konstruktionszeit-Kopie -- [290] wird NUR unter POSITIV_HAKEN1 geschrieben, der Bericht darf ihn sonst nicht als "0" ausgeben
	 ulong pos_zellen = 0ull, pos_mehrdeutig = 0ull; // ★ P1d: Positiv-Messarm je Fenster (Stichprobenzellen je Zaehlschritt, Wickelwaechter 292/293)
};
...
static void klemm_lesen(LBM& L, KlemmBilanz& K, const double t_si, const bool nach_warmup, const string& csv_pfad, const char* wo) {
	...
	if(!d->klemm_bilanz_on||L.get_D()!=1u) {
		// ★ Audit-Schleife 16.09.2026, Befunde B9/B10: hier wurde WORTLOS zurueckgekehrt. Folge (1) B10: bei CFD_KLEMM_BILANZ=0 oder D > 1
		// fehlten KLEMM-BILANZ, KLEMM-HUELLEN, POSITIV-BILANZ und ALLE KLEMM-BUDGET-Zeilen, ohne eine einzige Zeile im Log -- nicht
		// unterscheidbar von "alles in Ordnung". Folge (2) B9: die Wertpruefung von CFD_KLEMM_BUDGET sitzt in klemm_budget_modus(), das
		// hinter diesem return nie gerufen wurde -- CFD_KLEMM_BUDGET=7 wurde still ignoriert.
		(void)klemm_budget_modus(); // validiert den Wert auch ohne Instrument (bricht bei > 2 ab)
		if(!K.aus_gemeldet) { K.aus_gemeldet = true;
			print_info(string("KLEMM-INSTRUMENT ")+wo+" AUS ("+(!d->klemm_bilanz_on ? "CFD_KLEMM_BILANZ=0" : "D = "+to_string(L.get_D())+" > 1, Block-Tiling")+") -- keine KLEMM-BILANZ, keine Huellen-, Positiv- und Budgetzeilen in diesem Lauf. Die Klemmen selbst wirken weiter, sie werden nur nicht gemessen.");
		}
		return;
	}
	...
	for(uint b=0u; b<6u; b++) { B_r += (double)dlt[236u+b]*(2.0*ob[b]*S+1.0); B_u += (double)dlt[257u+b]*(2.0*(rho_huelle_aktiv() ? 3.0 : 2.1)*ob[b]*S+1.0); } // ★ Z2f: rho_max = numerische Huelle 2,99902
	...
	if(K.pos_modus>0u) { // ★ P1d: Wickelwaechter 292/293 -- je begrenzter Stichprobenzelle hoechstens S (1-s <= 1) bzw. 16 S (Kappe)
		...
		if(K.csv_pos.is_open()) {
			...
			K.csv_pos << "," << dlt[286] << "," << dlt[287] << "," << dlt[291] << "," << dlt[285] << "," << (double)dlt[292]/S << "," << (double)dlt[293]/S << "," << dlt[294] << "," << ((K.pos_haken==1u||K.pos_haken==3u) ? to_string(dlt[290]) : string("")) << "," << (pmd_?1:0) << "," << (pmd3_?1:0) << "\n" << std::flush;
		}
	}
}
...
// Die Masse wird auf den Durchfluss u_lat*Ny*Nz (rho = 1) je Schritt bezogen (Groessenbezug; das Nahfeld hat keinen Einlass). n_Schritte = Schritte DIESER Domaene.
// ★ 15.09.2026 Klemmen Z2c: Budget-Kennwerte der Phase "nach Warmlauf" je Domaene (Plan §2.4). dcd_m = Massenaequivalent ueber den
// Impulsfluss bei u_inf (Lagally, deklariertes Interim "netto bei u_inf"; Ablösebedingung: brutto reisst, netto nicht).
struct KlemmUrteil { bool pruefbar = false, kappung = false; ulong n = 0ull; double dcd_j = 0.0, dcz_j = 0.0, dcd_m = 0.0, dcd_m_brutto = 0.0, q_netto = 0.0, q_brutto = 0.0, rund_cd_j = 0.0, rund_cd_m = 0.0, rund_cd = 0.0, rund_cz = 0.0; string grund; }; // rund_* = Worst-Case-Rundungsschranke des Festkommas (Treffer/(2S)) als Cd/Cz (Z2c Pruefpass NIEDRIG 5)
// k(B) = sqrt((1+r)^2 - 1), r = sqrt(1-c4^2)/c4, c4(B) = sqrt(2/(B-1))*Gamma(B/2)/Gamma((B-1)/2): die Klemme darf die Gesamtunsicherheit
// sqrt(sigma^2+b^2) nicht staerker verschieben, als sigma-Dach selbst unsicher ist. k(4) = 1,011, k(8) = 0,786, k(16) = 0,634. Kein Handwert.
static double klemm_budget_k(const uint B) {
	const double c4 = sqrt(2.0/(double)(B-1u))*exp(lgamma(0.5*(double)B)-lgamma(0.5*(double)(B-1u)));
	const double r = sqrt(fmax(0.0, 1.0-c4*c4))/c4;
	return sqrt((1.0+r)*(1.0+r)-1.0);
}
static void klemm_budget_bewerten(const char* wo, const KlemmUrteil& U, const double sig_cd, const double sig_cz, const string& csv_pfad, const bool neu) {
	const uint modus = klemm_budget_modus(); // 0 = nicht bewerten, 1 = nur Warnung, 2 = Fehler am Fallende (Vorgabe, Uebergabe §1.4)
	if(modus==0u) return;
	const uint B = 4u; const double k = klemm_budget_k(B);
	const double lhs = fabs(U.dcd_j)+fabs(U.dcd_m), lhs_b = fabs(U.dcd_j)+fabs(U.dcd_m_brutto);
	string urteil;
	if(!U.pruefbar) urteil = "nicht pruefbar ("+U.grund+")";
	else if(!(sig_cd>0.0)) urteil = "nicht pruefbar (sigma_cd fehlt oder 0: weniger als 2B Samples nach Warmlauf, Facettenpfad aus oder konstante Reihe)"; // Z2c Pruefpass NIEDRIG 4
	else {
		const bool cz_da = sig_cz>0.0;
		const bool cd_ok = lhs<=k*sig_cd, cz_ok = !cz_da||fabs(U.dcz_j)<=k*sig_cz;
		urteil = (cd_ok&&cz_ok) ? "eingehalten" : "GERISSEN";
		if(!cd_ok||!cz_ok) {
			...
			// ★ Audit-Schleife 16.09.2026, Befund A-N5: die Zuordnung JE GROESSE benennen. Vorher stand nur die Summe da, und der Fall
			// "der Massenterm allein liegt unter seiner eigenen Rundungsschranke" (Haken 1: |dCd_m| 1,017 gegen rund_cd_m) blieb unsichtbar.
			if(cd_r&&fabs(U.dcd_m)<=U.rund_cd_m) urteil += " [davon der MASSENTERM |dCd_m| = "+to_string(fabs(U.dcd_m),6u)+" unter seiner eigenen Rundungsschranke "+to_string(U.rund_cd_m,6u)+" -- dieser Anteil ist Rauschen des Messgeraets, kein gemessener Effekt]";
			if(cd_r&&fabs(U.dcd_j)<=U.rund_cd_j) urteil += " [davon der IMPULSTERM |dCd_j| = "+to_string(fabs(U.dcd_j),6u)+" unter seiner eigenen Rundungsschranke "+to_string(U.rund_cd_j,6u)+"]";
			...
			const bool cd_r = lhs+U.rund_cd>k*sig_cd, cz_r = cz_da&&fabs(U.dcz_j)+U.rund_cz>k*sig_cz; // haelt das Urteil auch mit der Worst-Case-Rundung? (Pruefpass 2 NIEDRIG 1: je Groesse benannt)
			if(cd_r||cz_r) urteil += " (mit Worst-Case-Rundung nicht mehr sicher: "+string(cd_r ? "Cd "+to_string(U.rund_cd/sig_cd,3u)+" sigma" : "")+string(cd_r&&cz_r ? ", " : "")+string(cz_r ? "Cz "+to_string(U.rund_cz/sig_cz,3u)+" sigma" : "")+" -- Urteil unsicher)";
			if(!cz_da) urteil += " (Cz nicht pruefbar: sigma_cz fehlt -- nur Cd bewertet, Urteil unsicher)"; // Pruefpass 2 NIEDRIG 5: Plan §3 "nicht pruefbar" = Warnung
		}
		if(cd_ok&&lhs_b>k*sig_cd) print_warning(string("  KLEMM-BUDGET ")+wo+": netto eingehalten, BRUTTO gerissen ("+to_string(lhs_b/sig_cd,3u)+" sigma) -- Ablösebedingung des Interims \"netto bei u_inf\" ausgeloest: das Urteil haengt am Ort der Quellen (Plan §2.4, lokale Summen noetig).");
	}
	const string txt = string("  KLEMM-BUDGET ")+wo+" (nach Warmlauf, "+to_string(U.n)+" Schritte): |dCd_j| + |dCd_m| = "+to_string(fabs(U.dcd_j),6u)+" + "+to_string(fabs(U.dcd_m),6u)
		+(sig_cd>0.0 ? " = "+to_string(lhs/sig_cd,4u)+" sigma_cd" : string(""))+" (brutto "+(sig_cd>0.0 ? to_string(lhs_b/sig_cd,4u)+" sigma" : to_string(lhs_b,6u))+"), |dCz_j| = "+to_string(fabs(U.dcz_j),6u)
		+(sig_cz>0.0 ? " = "+to_string(fabs(U.dcz_j)/sig_cz,4u)+" sigma_cz" : string(" (sigma_cz fehlt)"))+"; Grenze k(4)*sigma = "+to_string(k,4u)+" sigma -> "+urteil;
	if(urteil.rfind("GERISSEN",0)==0) print_warning(txt+(modus==2u ? " -- Abbruch am Fallende (CFD_KLEMM_BUDGET=2)." : " (CFD_KLEMM_BUDGET=1: nur Warnung)."));
	else if(urteil.rfind("nicht",0)==0||urteil.find("unsicher")!=string::npos) print_warning(txt); // Z2c Pruefpass M3: "eingehalten, unsicher" ist eine Warnung (Plan §3)
	else print_info(txt);
	std::ofstream c(csv_pfad, neu ? std::ios::trunc : std::ios::app); c.precision(8);
	if(neu) c << "# Klemm-Budget (KLEMMEN-STUFE2-PLAN.md §2.4): |dCd_j|+|dCd_m| <= k(B)*sigma_cd, |dCz_j| <= k(B)*sigma_cz, B = " << B << ", k = " << k << "; nur Phase nach Warmlauf; dCd_m = Massenaequivalent bei u_inf (Interim); Q = Gitter-Masse je Schritt der Domaene, SENKE POSITIV (wie dCd_m; Plan §2.4 schrieb Quelle +); netto = |innen| + |BODEN/EINLASS_EQ| mit Vorzeichen des Gesamtnettos; Zeile Nah+Fern: Q leer (Fein- und Grobschritte nicht addierbar), n = Nahfeldschritte\n"
		<< "bereich,n_schritte,Q_netto,Q_brutto,dCd_j,dCz_j,dCd_m_netto,dCd_m_brutto,rund_cd,rund_cd_j,rund_cd_m,rund_cz,sigma_cd,sigma_cz,B,k,kappung,urteil\n"; // Q = Gitter-Masse je Schritt (Senke +)
	string urteil_csv = urteil; for(char& ch : urteil_csv) if(ch==',') ch = ';'; // Z2c Pruefpass NIEDRIG 2: kein Spaltenversatz
	const bool q_leer = string(wo)=="Nah+Fern";
	c << wo << "," << U.n << ",";
	if(q_leer) c << ",,"; else c << U.q_netto+0.0 << "," << U.q_brutto+0.0 << ",";
	c << U.dcd_j+0.0 << "," << U.dcz_j+0.0 << "," << U.dcd_m+0.0 << "," << U.dcd_m_brutto+0.0 << "," << U.rund_cd << "," << U.rund_cd_j << "," << U.rund_cd_m << "," << U.rund_cz << "," << sig_cd << "," << sig_cz << "," << B << "," << k << "," << (U.kappung?1:0) << "," << urteil_csv << "\n"; // +0.0: keine "-0"
}
static KlemmUrteil berichte_klemmbilanz(KlemmBilanz& K, const char* wo, Units u, const double qA, const float u_lat, const uint Ny, const uint Nz, const double sigma_cdrest, const char* sigma_name = "sigma(cd_rest, Block-SEM 4)") {
	KlemmUrteil U;
	if(!K.init) { U.grund = "Instrument aus"; return U; }
	...
	for(uint ph=0u; ph<2u; ph++) {
		...
		if(n==0ull) { print_info(string("  KLEMM-BILANZ ")+wo+" ("+pn+"): keine Schritte in dieser Phase."); if(ph==1u) U.grund = "keine Schritte nach Warmlauf"; continue; }
		...
		if(ph==1u) { // ★ Z2c: Budget-Kennwerte (Masse inkl. BODEN/EINLASS_EQ, S0d)
			// Z2c Pruefpass 2 (MITTEL): Innen- und BODEN/EINLASS_EQ-Nettomasse NICHT mit Vorzeichen verrechnen -- gegenlaeufige Anteile
			// wuerden sich aufheben (Fehlerklasse M2). Betrag = |Q_innen| + |Q_eq|, Vorzeichen = das des Gesamtnettos (nur Anzeige).
			...
			const double hr_ex = su(a,236u,6u), hu_ex = su(a,257u,6u); // Pruefpass 2 NIEDRIG 2: ungegatete Dekadensummen statt Klassenzaehler (KLEMM_HAKEN3 gatet 221+kk)
			// ★ Audit-Schleife 16.09.2026, Befund A-N5: die Rundungsschranke wird GETRENNT je Groesse gefuehrt. Vorher stand hier EINE Zahl
			// fuer Impuls und Masse zusammen; der Fall "der Massenterm allein liegt unter seiner EIGENEN Rundungsschranke" (Haken 1: |dCd_m|
			// 1,017 gegen rund_cd 1,128) liess sich damit nicht benennen. rund_cd bleibt die Summe -- die Urteilslogik aendert sich NICHT.
			U.rund_cd_j = fabs((double)u.si_F((float)(hu_ex/(2.0*S)/(double)n)))/qA;
			U.rund_cd_m = fabs((double)u.si_F((float)((double)u_lat*(hr_ex+a[266])/(2.0*S)/(double)n)))/qA;
			U.rund_cd = U.rund_cd_j+U.rund_cd_m; // Worst-Case je Buchung 1/(2S)
			U.rund_cz = fabs((double)u.si_F((float)(hu_ex/(2.0*S)/(double)n)))/qA;
			U.kappung = a[265]>0.0; U.pruefbar = K.mehrdeutig==0ull; if(!U.pruefbar) U.grund = "Wickelwaechter MEHRDEUTIG in mindestens einem Fenster, auch im Warmlauf (konservativ, Z2c Pruefpass NIEDRIG 6)";
	...
	if(K.pos_modus>0u) { // ★ 15.09.2026 Klemmen Stufe 1 P1d: Positiv-Messarm je Phase (gleiche Fenster wie die Klemm-Bilanz)
		...
		for(uint ph=0u; ph<2u; ph++) {
			...
			print_info(string("    Koinzidenz Dichteklemme ")+to_string(a[286], 0u)+" ("+proz_(a[286])+"), u-Klemme "+to_string(a[287], 0u)+" ("+proz_(a[287])+"); TYPE_E-Kandidaten "+to_string(a[291], 0u)+"; negativ geladen "+to_string(a[285], 0u)
				+(K.pos_modus==2u ? "; angewandt: Summe (1-s) "+to_string(a[292]/16384.0, 4u)+", Summe |df| "+to_string(a[293]/16384.0, 6u)+", Kappung "+to_string(a[294], 0u)+((K.pos_haken==1u||K.pos_haken==3u) ? ", Selbstpruefung [290] "+to_string(a[290], 0u) : string(", Selbstpruefung [290] nicht instrumentiert (nur unter CFD_POSITIV_HAKEN 1/3 gebaut)")) : string("")));
		}
...
void berichte_dichteklemme(LBM& L, const char* wo, ulong& summe, const float u_lat=0.0f) { // u_lat nur fuer die Freistrom-Ansage unter U_FP16; 0 = nicht bekannt, dann entfaellt sie
	...
	if(L.lbm_domain[0]->klemm_bilanz_on) { // ★ 15.09.2026 Klemmen S0b (KLEMMEN-STUFE0-PLAN.md §6): Ist=Soll des Messinstruments am Laufende
		...
		for(uint k : {0u, 1u, 221u, 222u, 223u, 224u, 225u, 236u, 237u, 238u, 239u, 240u, 241u}) if(v[k]>=4026531840ull) satt_r = true;
		for(uint k : {28u, 242u, 243u, 244u, 245u, 246u, 257u, 258u, 259u, 260u, 261u, 262u}) if(v[k]>=4026531840ull) satt_u = true;
		...
		{ const LBM_Domain* d0 = L.lbm_domain[0]; // Pruefpass S0b MITTEL 2: ohne set_force_bbox (Kugel, Kanal) ist die F-BBox die ganze Domaene
		...
		if(klemm_haken_env()==3u&&r01>0ull) { // ★ Nachpruefung 16.09. (M2): NUR rho-Treffer -- die Klassenluecke n%7 sitzt auf der rho-Seite. Mit "r01+v[28]>0" haette eine Domaene mit u-Treffern, aber ohne rho-Treffer, das triviale 0==0 als "Negativtest feuert nicht" gewertet und einen korrekten Lauf abgebrochen. // ★ Audit-Schleife 16.09.2026, Befund C-M3: Haken 3 hatte als EINZIGER Testhaken keinen
			...
			if(satt_r) print_warning(string("  KLEMM-BILANZ ")+wo+": HAKEN 3 gesetzt, aber die rho-Seite ist GESAETTIGT -- die Klassenluecke ist hier NICHT PRUEFBAR (kein Freispruch). Lauf kuerzen oder Zaehltakt ausduennen.");
			else if(rk==r01&&rd==r01) { print_warning(string("  KLEMM-BILANZ ")+wo+": HAKEN 3 gesetzt, aber die Klassensumme "+to_string(rk)+" stimmt mit [0]+[1] = "+to_string(r01)+" ueberein -- die eingebaute Klassenluecke (n%7) beanstandet NICHTS, der Negativtest feuert nicht. Abbruch am Fallende.");
				klemm_bilanz_verletzt = true; }
		}
		if((!satt_r&&(rk!=r01||rd!=r01))||(!satt_u&&(uk!=v[28]||ud!=v[28]))) {
		...
		{ // ★ 15.09.2026 Klemmen Z2b (KLEMMEN-STUFE2-PLAN.md §4): Huellenzaehler
			...
			print_info(string("  KLEMM-HUELLEN ")+wo+(rho_huelle_aktiv() ? ": [RHO_HUELLE] Konsistenzhuelle unten/oben verlassen [298]/[299] = "+to_string(v[298])+"/"+to_string(v[299])+" (nur gezaehlt), " : (rho_huelle_env()>0u ? string(": [RHO_HUELLE durch CFD_KLEMM_HAKEN 1/3/4 UEBERSTEUERT -- RHO_HUELLE ist nicht emittiert, [298]/[299] existieren in diesem Bau NICHT und werden darum nicht gedruckt (Audit 16.09.2026, Befund B2/C-M1)], ") : string(": ")))+(tor_huelle_env()>0u ? string("[TOR_HUELLE] ") : string(""))+"u-Komponentenhuelle [295] "+to_string(v[295])+(L.lbm_domain[0]->u_klemme>0u ? string(", Betragshuelle [296] ")+to_string(v[296])+" (Soll [28] = "+to_string(v[28])+", CFD_U_KLEMME=1)," : " (Soll [28] = "+to_string(v[28])+"), Betragshuelle [296] "+to_string(v[296])+",")+" davon Diagonalluecke [297] "+to_string(v[297])+" (Betragshuelle verletzt, Komponentenhuelle nicht), Lift-rho ausserhalb der Bildhuelle [300] "+to_string(v[300])+(klemm_haken_env()==5u ? " (Haken 5: Soll > 0 in Domaenen mit Lift-Besuchen [215] = "+to_string(v[215])+"; Tor [270] "+to_string(v[270])+")" : " (Soll 0; Tor [270] "+to_string(v[270])+")"));
			string hv_;
			if(!satt_h) {
			...
			if(klemm_haken_env()==5u&&rho_huelle_aktiv()&&v[298]+v[299]==0ull) hv_ += " Haken 5 unter RHO_HUELLE: Soll [298]+[299] > 0, Ist 0 (Positivtest der Konsistenzhuellen-Zaehler);";
			...
			{ // ★ Audit-Schleife 16.09.2026, Befund B1 (HOCH): KONSTANTENSPIEGEL des Tors. CFD_TOR_HUELLE hatte als einzige Abnahme eine
			  // Null ([270] = 0) -- nicht unterscheidbar von einem Lauf, in dem die Emission gar nicht griff. Der Kernel schreibt die
			  // UEBERSETZTEN Torgrenzen nach [301]/[302] zurueck; hier steht die UNABHAENGIGE Host-Rechnung derselben Grenzen (Zwilling zur
			  // Emission lbm.cpp, absichtlich getrennt gerechnet -- stimmen sie ueberein, ist der Wirkpfad belegt).
		...
		if(L.lbm_domain[0]->positiv_modus>0u) { // ★ 15.09.2026 Klemmen Stufe 1 P1b (KLEMMEN-STUFE1-PLAN.md §4, Nachtrag P1b): Ist=Soll des Positiv-Messarms
			...
			if(L.lbm_domain[0]->positiv_modus==2u) { // ★ 15.09.2026 P1c: Wirkung und Selbstpruefung
				...
				// ★ Audit-Schleife 16.09.2026, Befund B3: WIRKPFADBELEG des Schalters. [273] zaehlt K0-Kandidaten in beiden Stellungen gleich --
	...
	if(LBM_Domain::s_sgs_diag) {
		...
		if(ges>0ull) {
			...
			// Oberen Schwanz aufloesen (Slots 45..48)
			{
	...
	{	// ★ Pruefbefund 3-B/3-A (2026-08-25): WICKELWAECHTER fuer ALLE uebrigen Slots, unabhaengig
		...
		for(uint d=0u; d<L.get_D(); d++) { L.lbm_domain[d]->rho_clamp_hits.read_from_device();
			for(uint sl=0u; sl<(uint)L.lbm_domain[d]->rho_clamp_hits.length(); sl++) { const ulong v=(ulong)L.lbm_domain[d]->rho_clamp_hits[sl];
				...
				// ★ 04.09.2026 (Diff-Pruefung H3): Slot 75 (MESSNUR-Wirkpfad) steht in DERSELBEN Kernelzeile
				// wie Slot 7 (kernel.cpp:1974) und ist der EINZIGE nicht saettigende Zaehler im Bereich 68..95.
				// Die Erweiterung dieser Schleife auf die Pufferlaenge hat ihn erstmals freigelegt -- ohne die
		...
		if(mx7>3865470566ull) print_warning(string("  Zaehler ")+wo+": WIRKPFAD-Slot 7/75 steht bei "+to_string(mx7)
	...
	// Stichproben, keine Ereigniszahlen. [BERICHTIGT 15.09.2026: Slot 28 ist UNGEGATET und saettigend (kernel.cpp u-Klemme); Slot 29 (SPONGE) ist ebenfalls ungegatet und saettigend (kernel.cpp SPONGE-Block), Pruefpass S0b.]
	{
...
// ★ Z2c Pruefpass (M1): budget_folgt = dieser Fall bewertet das KLEMM-BUDGET wirklich (Instrument lief UND CFD_KLEMM_BUDGET > 0) --
// nur dann verweist das Fazit darauf; sonst bleibt die Warnung. Kanal, Fahrzeug-Einzelgitter und Fernfeld-Diagnose bewerten kein Budget.
void dichteklemme_fazit(const ulong summe, const bool budget_folgt=false) {
#ifdef RHO_CLAMP
...
static void remesh_facetten_diag(LBM& L, const uint Nx, const uint Ny, const uint Nz,
	...
	const uint quetsch_stufe=env_u("CFD_REMESH_QUETSCH", 0u); if(quetsch_stufe>0u) print_info("REMESH: CFD_REMESH_QUETSCH="+to_string((ulong)quetsch_stufe)+" (Quetschkanten-Vertices festhalten) -- D12.");
	...
	const uint bk_alt=env_u("CFD_REMESH_BINKEY_ALT", 0u); if(bk_alt>0u) print_warning("REMESH: CFD_REMESH_BINKEY_ALT=1 -- ALTER Bin-Key (Aliasing-Arm, nur fuer A/B) -- D12.");
...
static void zensus_statische_klassen(LBM& L, const std::vector<Facette>& FF, const uint Nx, const uint Ny,
	...
	for(const Facette& f : FF) {
		...
		for(uint i=1u; i<19u; i++) {
			...
			const bool kernel_link = (L.flags[nn]&(TYPE_S|TYPE_E))==TYPE_S; // Maske wie kernel.cpp:2100
			// ★ 04.09. (Diff-Pruefung): die MASKE ist die des Kernels, die NACHBARRICHTUNG nicht. Der Kernel
			// gattert Link i am Nachbarn n - c_i (ueber j[opposite(i)]), dieser Zensus an n + c_i -- die
			// Linkmenge ist also das Spiegelbild. Fuer ALLES, was hier gerechnet wird, ist das folgenlos:
			// |Menge|, S0 und G = Sum 6 w c c^T sind gerade in c, und S1 geht nur als S1 S1^T ins Downdate.
			// SCHARFE FALLE fuer spaeter: wer diesen Zensus auf alpha, beta3 oder das Delta-m-Leck
			// erweitert -- alle LINEAR in S1 -- bekommt ein gekipptes Vorzeichen.
		...
		auto klassifiziere=[&](const double M[3][3], bool* entk_aus, double* verh_aus, double* lmax_aus) {
			...
			if(entk_aus) *entk_aus=ent; if(verh_aus) *verh_aus=vh; if(lmax_aus) *lmax_aus=lmax;
	...
	print_info("   FLACKERN: "+to_string(n_flacker)+" Facetten ("+to_string((float)(100.0*(double)n_flacker/d),2u)
		...
		+"ACHTUNG (Diff-Pruefung 04.09.): daraus folgt KEIN Hebel. t1 ist nicht frei waehlbar -- es ist "
		+"u_t/|u_t| (kernel.cpp:2060), also die Richtung, in der das Wandschubziel R1 = -def_fac_tau*twe "
		+"aufgepraegt wird. t1 auf den Hauptvektor zu drehen wuerde das physikalische Ziel mitdrehen. "
		+"Die Zahl misst nur, wie basisempfindlich die Einstufung ist -- am Fahrzeug sind es 2 Facetten.");
	...
	if(n_praedikat_diff>0ull) print_warning("["+ort+"] PRAEDIKAT-DIVERGENZ: bei "+to_string(n_praedikat_diff)+" von "+to_string(ges)+" Facetten ("
		...
		+" (Kernel-Gate (flags&0x03)==TYPE_S gegen Bau-Gate flags=="+to_string((ulong)wand_flag)+" dezimal"+"). Links gesamt Kernel "
...
void main_setup_kanal() {
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) {
		...
		lbm.alloc_facetten(FF, nullptr, env_u("CFD_SGS_GDIAG", 0u), env_u("CFD_SGS_FDWAND", 0u), env_u("CFD_SGS_SISM", 0u));
		lbm.lbm_domain[0]->alloc_sgs_band(&lbm.flags[0], Nx, Ny, Nz, env_u("CFD_SGS_BAND", 0u)); // ★ 08.09. SGS-BAND (Kanal: dieselbe Kette wie dd)
		zensus_statische_klassen(lbm, FF, Nx, Ny, Nz, TYPE_S, (env_u("CFD_FACETTEN",0u)>=3u&&env_u("CFD_FAC_ALPHA",0u)>=2u&&env_u("CFD_FAC_MASSE_ALLE",0u)==0u), "Kanal", out_dir); // Parameter statt Statik (02.09.)
	...
	for(ulong step=0ull; step<n_steps; step+=(ulong)regel_alle) { // Audit-Nacharbeit 18: letzter Chunk gekappt, vorher bis zu 99 Schritte Ueberzug
		...
		sism_sbar_zeile(lbm.lbm_domain[0], lbm.get_t(), out_dir); // ★ 07.09. SISM: Sbar-Zeitreihe je Chunk (no-op ohne CFD_SGS_SISM)
	...
	// ★ 04.09.2026 (Instrument-Fehler): Feld-Hash VOR die abbrechenden Berichte -- die X-Arme hatten keinen Hash,
	// weil ein print_error im Bericht vor der Hash-Zeile feuerte. Die Berichte aendern u nicht, der Wert ist derselbe.
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) { // ★ Stufe 2: Wirkpfad-Nachweis, Soll EXAKT (F7)
		...
		if(env_u("CFD_FAC_ELIBB",0u)==1u&&(ulong)lbm.lbm_domain[0]->rho_clamp_hits[67]==0ull) print_warning("CFD_FAC_ELIBB=1 angefordert, aber Slot 67 = 0 -- die Blende wird nie erreicht (stiller No-Op? S4)."); // ★ S4: VOR dem Pur-Block, sonst unerreichbar (Diff-Pruefung B3)
		...
		// ★ 10.09.: DIE DIAGNOSTIK-ABNAHME STEHT VOR DEM K-KRITERIENBLOCK. Grund, am selben Tag
		// bezahlt: K2 unten ist ein print_error und damit exit(1); der Lauf ns_an endete daran,
		// BEVOR die Wirkpfad-Abnahme des Schalters ueberhaupt aufgerufen wurde -- der Schalter
		// wirkte nachweislich (Feld-Hash verschieden, cf 0,0079515 -> 0,0072807), aber Ist=Soll
		// war nicht ablesbar. Eine Abnahme, die ein fremdes Abbruchkriterium verschlucken kann,
		// ist keine. Derselbe Einwand trifft pruefe_band_wirkpfad/pruefe_sism_wirkpfad/
		// bericht_vandriest weiter unten -- als Befund notiert, hier nicht mitgeaendert.
		if(lbm.lbm_domain[0]->nut_skal!=1.0f) pruefe_nut_skal_wirkpfad(lbm.lbm_domain[0]->rho_clamp_hits.data(), lbm.lbm_domain[0]->nut_skal, "Kanal");
...
static void lese_rho_rand_einzelgitter(const string& fall) {
	...
	{ const uint hk_ = env_u("CFD_RHO_RAND_TESTHAKEN", 0u); if(hk_!=0u&&hk_!=1u&&hk_!=5u&&hk_!=7u) print_error("CFD_RHO_RAND_TESTHAKEN="+to_string(hk_)+" ist im Fall "+fall+" nicht gebaut (gueltig: 1 C0-Waechter, 5 TYPE_E im Inneren -> Slot 216, 7 Soll+1 in der Abnahme)."); }
	if(env_f("CFD_SLICE_DT", 0.0f)<=0.0f) print_error("CFD_RHO_RAND ("+fall+") ohne Slices (CFD_SLICE_DT <= 0): der Ausgabepfad liefe nie, die Abnahme haette keinen gezaehlten Aufruf (Pruefpass C2c NIEDRIG 7).");
...
static void berichte_rho_rand(LBM& L, const string& wo, const bool mit_lift) {
	...
	d->finish_queue(); d->rho_clamp_hits.read_from_device();
	const ulong h[7] = {(ulong)d->rho_clamp_hits[204], (ulong)d->rho_clamp_hits[205], (ulong)d->rho_clamp_hits[211], (ulong)d->rho_clamp_hits[215], (ulong)d->rho_clamp_hits[216], (ulong)d->rho_clamp_hits[219], (ulong)d->rho_clamp_hits[220]};
	const ulong haken7 = env_u("CFD_RHO_RAND_TESTHAKEN", 0u)==7u ? 1ull : 0ull; // Negativtest der Vergleiche: Soll um 1 verfaelscht -> MUSS reissen
	if(haken7) print_warning("RHO_RAND-TESTHAKEN 7: alle Soll-Werte der Abnahme um 1 erhoeht -- Soll: Abnahme verletzt.");
	bool ok = true, teil = false;
	...
	print_info("  Geraete-rr_idx gegen Host-rr_idx_host an TYPE_E: "+to_string(L.rho_aus_rr_verglichen)+" verglichen, "+to_string(L.rho_aus_rr_abw)+" Abweichungen (Soll > 0 / 0)");
	if(L.rho_aus_rr_verglichen==0ull||L.rho_aus_rr_abw+haken7!=0ull) ok = false;
	if(zaehlschritt_im_lauf) {
		...
		// stream_collide zaehlt jede Zelle, die weder reines TYPE_S noch reines TYPE_E ist -- also Fluid UND TYPE_MS (0x03).
		// Die Host-Flags tragen TYPE_MS nur, wenn ein Setup die Geraete-Flags zurueckgelesen hat (Kugel: ja) -- deshalb
		// wird hier auf "nicht S, nicht E" geprueft, nicht auf "weder S- noch E-Bit" (CPU-Sprosse 15.09.: 212 MS-Zellen je Schicht).
		for(ulong n=0ull; n<NN; n++) { const uchar f = L.flags[n]&(TYPE_S|TYPE_E); if(f!=TYPE_S&&f!=TYPE_E) { soll_alle++; if((uint)(n%(ulong)Nx)+2u>=Nx) soll_maske++; } }
		print_info("  Slots 204/205 (Schreibmaske): "+to_string(h[0])+"/"+to_string(h[1])+" (Soll "+to_string(soll_alle-soll_maske)+"/"+to_string(soll_maske)+")");
		if(h[0]+h[1]!=soll_alle+haken7||h[1]!=soll_maske+haken7) ok = false;
	}
...
static void pruefe_rho_rekonstruktion(LBM& lbm, const uint Nx, const uint Ny, const uint Nz, const uint achse, const uint pos, const float u_lat, const string& out_dir, const string& fall) {
	...
	for(ulong g=0ull; g<NP; g++) {
		...
		if(k==0u) {
			...
			if(ist_facette(n)) k = 6u;
			else if(d->fac_N>0ull&&ist_band2(n)) k = 8u;
	...
	ulong n_ms_asym=0ull, ms_asym_mms_reisst=0ull, ms_asym_alt_reisst=0ull, ms_formel_falsch=0ull, haken2_reisst=0ull, haken2_basis=0ull, n_klemm_t0=0ull, n_alt_auf_grenze=0ull;
	...
	for(ulong g=0ull; g<NP; g++) {
		...
		const bool klemm_vor = wand_klasse&&(r_vor<=RHO_CLAMP_MIN||r_vor>=RHO_CLAMP_MAX); // Pruefpass 2 C2a, NIEDRIG 2: NUR das Pufferwort -- "alt auf der Grenze" haette eine kaputte Ausgabe still ausgenommen
		if(wand_klasse&&!klemm_vor&&(alt[(size_t)(4ull*g)]<=RHO_CLAMP_MIN||alt[(size_t)(4ull*g)]>=RHO_CLAMP_MAX)) n_alt_auf_grenze++; // Soll 0 (getrennt gezaehlt)
		...
		if(k==0u||k==3u||k==6u||k>=8u) {
			...
			if(!klemm_vor) {
				...
				if(k==3u) {
					...
					const float mv = mms[(size_t)(4ull*g)];
					if(fabs(sc)>2.0*S&&mv>RHO_CLAMP_MIN&&mv<RHO_CLAMP_MAX) { // Pruefpass 2 C2a, NIEDRIG 1: erst ab 2S ist "mms reisst S" garantiert (|mms-rho| >= |sc|-|alt-rho| > S); mms auf der Klemmgrenze waere Fehlalarm
						n_ms_asym++; if(dm>S) ms_asym_mms_reisst++; if(da>S) ms_asym_alt_reisst++; if(dm/S>max_quote_mms_ms) max_quote_mms_ms = dm/S;
						const double dmm = (double)mv-(double)alt[(size_t)(4ull*g)]; // = Summe der Korrekturen, bis auf Rundung
	...
	print_info("  Nachkollisionssumme auf der Klemmgrenze bei ungeklemmtem Puffer: "+to_string(n_alt_auf_grenze)+" (Soll 0)");
	if(n_alt_auf_grenze>0ull) ok = false;
	...
	if(n_ms_asym>0ull) {
		print_info("  MS asymmetrisch (|Korrektur| > 2S): "+to_string(n_ms_asym)+" Zellen; MIT Korrektur reissen S "+to_string(ms_asym_mms_reisst)+" (Soll alle, max Quote "+to_string(max_quote_mms_ms,3u)+"), OHNE "+to_string(ms_asym_alt_reisst)+" (Soll 0), Host-Formel falsch "+to_string(ms_formel_falsch)+" (Soll 0)");
		if(ms_asym_alt_reisst>0ull||ms_asym_mms_reisst!=n_ms_asym||ms_formel_falsch>0ull) ok = false;
	} else print_info("  MS asymmetrisch (|Korrektur| > 2S): 0 Zellen -- MS-Varianten auf dieser Ebene NICHT TRENNSCHARF (Entscheidung stuetzt sich auf die Herleitung; Haken 3 folgt in C2c)");
...
void main_setup_kugel() {
	...
	LBM_Domain::s_sponge_n = 0u; LBM_Domain::s_sponge_a = 3000.0f; LBM_Domain::s_sponge_wmin = 0.5f; LBM_Domain::s_sgs_wandfrei = env_u("CFD_SGS_WANDFREI", 0u)>0u; LBM_Domain::s_sgs_guo = env_u("CFD_SGS_GUO", 1u)>0u; LBM_Domain::s_sgs_diag = env_u("CFD_SGS_DIAG", 0u)>0u; LBM_Domain::s_sgs_diag_ab = (ulong)env_schritte("CFD_SGS_DIAG_AB", 0u); // alle drei, damit keine Instanz einen Wert der vorigen erbt
	...
	const uint  sample_every = max(1u, env_schritte("CFD_SAMPLE_EVERY", 10u)); // 0 waere eine Endlosschleife -- hier klemmen, nicht im Helfer
	...
	if(lbm.lbm_domain[0]->rho_rand_on) { // ★ 15.09. RHO_RAND C2c: C0-Waechter (R1-Abdeckung aller rho-Leser) auch am Pruefstand
		const bool haken5 = env_u("CFD_RHO_RAND_TESTHAKEN", 0u)==5u;
		if(haken5) { // ★ C2c Haken 5 (Pruefpass M2): eine Fluidzelle im Inneren wird TYPE_E -> stream_collide liest den Papierkorb, Slot 216 > 0 -> Abnahme MUSS reissen
			const ulong nh = (ulong)(Nx-10u)+((ulong)(Ny/2u)+(ulong)(Nz/2u)*(ulong)Ny)*(ulong)Nx;
			lbm.flags[nh] = TYPE_E; lbm.u.x[nh] = u_lat; lbm.u.y[nh] = 0.0f; lbm.u.z[nh] = 0.0f;
	...
	// ★ Audit-Schleife 16.09.2026, Befund B5: dieser Waechter stand VOR dem Klemmenbericht und endet auf print_error -> exit(1);
	// ein Slice-Fehlalarm haette den ganzen Klemmenblock des Kugelfalls verschluckt. Reihenfolge wie im Kanal- und dd-Fall:
	// erst berichten, dann die Wirkpfad-Waechter (cd_w/cz_w werden oben gebraucht, darum wandert der Waechter, nicht der Bericht).
	...
	if(env_u("CFD_FACETTEN", 0u)>0u) { // ★ Stufe-2-Commit 3: Wirkpfad Ist=Soll + tau-Akkumulator an der Kugel
		...
		if(env_u("CFD_FACETTEN",0u)>=3u) { const uint* H=lbm.lbm_domain[0]->rho_clamp_hits.data(); pruefe_rueckfall_buchung(H[69],H[10],H[13],H[15],H[16],H[64],H[124],LBM_Domain::s_fac_uw,LBM_Domain::s_fac_satgate,"Kugel",LBM_Domain::s_fac_masse_alle==3u?(ulong)H[94]:0ull); pruefe_kraftpfad(H[70],H[69],H[7],H[9],H[17],H[71],LBM_Domain::s_fac_kraft,"Kugel"); pruefe_kaskade(H,"Kugel",LBM_Domain::s_fac_messnur>0u,LBM_Domain::s_fac_pinv>0u,LBM_Domain::s_fac_uw, LBM_Domain::s_fac_rdiag); pruefe_masse_alle(H,LBM_Domain::s_fac_masse_alle>0u,LBM_Domain::s_fac_messnur>0u,LBM_Domain::s_fac_kraft,"Kugel",LBM_Domain::s_fac_masse_alle==3u,LBM_Domain::s_fac_uw); bericht_zielerfuellung(H,(ulong)H[7],(ulong)H[9],(ulong)H[17],(ulong)H[69],"Kugel",LBM_Domain::s_fac_uw); bericht_gate_kreuztabelle(H,"Kugel",LBM_Domain::s_fac_masse_alle==3u,LBM_Domain::s_fac_satgate,LBM_Domain::s_fac_messnur>0u,LBM_Domain::s_fac_uw);
			...
			// ★ C5 (07.09.): dieselbe Luecke wie bei bericht_klassen am 05.09. -- Kanal (3637) und Nahfeld (7955) rufen bericht_gdiag, die Kugel nicht. Damit war die g-Diagnose am GEKRUEMMTEN Koerper unmessbar, und genau dort liegt die offene Frage (WALE/Sigma sind nur an der EBENEN Kanalwand vermessen).
			}
	...
	if(env_u("CFD_SGS_BAND",0u)>0u) pruefe_band_wirkpfad(lbm.lbm_domain[0], lbm.get_t(), "Kugel"); // ★ 08.09. SGS-BAND
	...
	if(env_u("CFD_SGS_VANDRIEST",0u)>0u) bericht_vandriest(lbm.lbm_domain[0], LBM_Domain::s_sgs_vd_aplus, "Kugel"); // ★ 08.09. van Driest: Wirkpfad, D^2-Histogramm, Ist=Soll Geraet gegen Host-y+
	print_info("---------------------------------------------------------------");
	...
	berichte_rho_rand(lbm, "Kugel", false); // ★ 15.09. RHO_RAND C2c (kein Lift an der Kugel)
...
static void main_setup_fahrzeug_dd() {
	...
	{	const bool s_ = getenv("CFD_SLICE_NEAR_STEPS")!=nullptr, d_ = getenv("CFD_SLICE_DT")!=nullptr;
		const ulong ns_ = s_ ? (ulong)env_schritte("CFD_SLICE_NEAR_STEPS", 5000u) : (d_ ? 0ull : 5000ull);
	...
	uint nahfeld_uw = 0u;
	uint nahfeld_rdiag = 0u; // ★ B-1/C2 (Auditoren B+C, 07.09.): Wert wird ERST HINTER DEM PARSEN gesetzt -- hier stand die Zuweisung und war damit im dd-Fall konstruktiv 0
	...
	nahfeld_messnur = LBM_Domain::s_fac_messnur>0u; // ★ 03.09.: gleiche Falle fuer das MESSNUR-Gating des Slot-72-Lesers
	  ...
	  LBM_Domain::s_fac_uw = (env_u("CFD_FACETTEN",0u)>=3u) ? min(1u, env_u("CFD_FAC_UW", 0u)) : 0u;
	  LBM_Domain::s_fac_uw_sn = env_u("CFD_FAC_UW_SN", 0u)>0u;
	// ★★ C1/C2 (Auditoren B+C, 07.09.): DIESE ZWEI KOPIEN MUESSEN HINTER DEM PARSEN STEHEN.
	//   Beide lasen ihre Statik vorher -- nahfeld_uw elf Zeilen zu frueh, nahfeld_rdiag 209 Zeilen.
	//   Folge waren zwei tote Waechter im dd-Fall: der RDIAG-No-Op-Waechter konnte am Fahrzeug nie
	//   feuern, und ein u_w-Fahrzeuglauf waere in pruefe_kaskade per exit(1) um seinen Endbericht
	//   gekommen -- woertlich die Falle, vor der die Zeile bei nahfeld_satgate selbst warnt.
	nahfeld_uw = LBM_Domain::s_fac_uw; nahfeld_rdiag = LBM_Domain::s_fac_rdiag;
	  if(env_u("CFD_FAC_UW",0u)>1u) print_error("CFD_FAC_UW kennt derzeit nur 0 und 1. Modus 2 (nu_t aus dem gemessenen Feld, fac_wfd) ist bewusst NICHT gebaut: er koppelt an CFD_SGS_FDWAND, braucht ein neues Kernelargument und ist im Kugelfall nicht verdrahtet -- eigener Schritt, eigene Variable.");
	  if(LBM_Domain::s_fac_uw>0u) {
	  	if(LBM_Domain::s_fac_masse_alle==0u) print_error("CFD_FAC_UW ohne CFD_FAC_MASSE_ALLE: u_w erzeugt an schraegen Facetten den Massenfluss 6(S1.u_w), weil S1 dort nicht parallel zu n steht (gemessene Fehlausrichtung 5-14 Grad). Ohne Kompensation leckt der Arm. MASSE_ALLE=1 verteilt sie impulsfrei ueber alle 19 Richtungen (Sum w_i c_i = 0).");
	  	if(LBM_Domain::s_fac_masse_alle==2u) print_error("CFD_FAC_UW mit CFD_FAC_MASSE_ALLE=2: Modus 2 ist am 04.09. verworfen (Bulk-Mode, Geschwindigkeitsklemme x9300, f0 <= 0 bei 2 % der Zellen).");
	...
	{ // ★ TODO 2 Schritt 2 (12.09.2026): das FERNFELD auf denselben Stand wie das Nahfeld
	  ...
	  const uint se_ = max(1u, env_schritte("CFD_SAMPLE_EVERY", 25u));
	...
	{ const uint ffc = env_u("CFD_FERN_FACETTEN", 0u);
	  ...
	  // ★ 04.09.2026 (Diff-Pruefung H2): s_fac_masse_alle war die EINZIGE der 45 facettenbezogenen
	  // Statiken, die hier nicht fuer das Grobgitter neu gesetzt wurde. lbm_c haette den Schalter vom
	...
	const uint  sample_every = max(1u, env_schritte("CFD_SAMPLE_EVERY", 25u)); // in groben Schritten
	...
	const ulong slice_ns = kad_s_gesetzt ? (ulong)env_schritte("CFD_SLICE_NEAR_STEPS", 5000u) : (kad_d_gesetzt ? 0ull : 5000ull);
	...
	print_info("[STOPP/ZIEL] wirksame Dateien: "+stop_datei+" | "+ziel_datei+" (D16: bei Parallellaeufen ueber CFD_STOP_DATEI/CFD_ZIEL_DATEI trennen).");
	...
	// Schale. Dazu der gemessene freie Speicher aus dem Debugfs oder aus /proc/*/fdinfo, der als einziger den
	...
	{	const ulong belegt_f = (ulong)lbm_f.lbm_domain[0]->get_device().info.memory_used;
		...
		const ulong frei_g   = vram_frei_gemessen(kap_f); // ★ 12.09.: Kapazitaet uebergeben, sonst kann der fdinfo-Weg keinen Freiwert bilden
		...
		if(frei_g>0ull) print_info("   GEMESSEN frei auf der Karte: "+to_string(frei_g)
			+" MB ("+string(vram_quelle())+" -- enthaelt den Desktop-Anteil, im Gegensatz zur Rechnung oben)."
			...
			" von opencl.hpp:170. Weder das Debugfs noch /proc/*/fdinfo trugen; sie sieht den Desktop-Anteil NICHT.");
	}
	...
	// ★ 12.09.2026 (Heiko): LEISTUNG AB EINER MARKE, nicht ueber den ganzen Lauf. Der Gesamtindex
	// traegt den Aufbau mit -- Voxelierung, Facettenbau, Remesh-Diagnostik, die erste Kopplung --
	// und der ist bei 4 mm ein zweistelliger Prozentsatz der Wanduhr. Fuer den Vergleich ZWISCHEN
	// Forks und zwischen Armen zaehlt der eingeschwungene Durchsatz. Marke per CFD_PERF_AB in
	// physikalischen Sekunden, Vorgabe 0,100 s. Beide Zahlen werden am Ende berichtet, damit
		...
		const ulong ns_ist = (outer+1ull)*(ulong)ratio; // exakte Near-Steps: BEIDE Pfade laufen ratio feine Schritte je Outer (Plan 27.08., setup.cpp:4699/4717)
			...
			if(slice_jetzt) {
				...
				if(slice_pruef) {
					...
					if(lbm_f.lbm_domain[0]->rho_rand_on&&env_u("CFD_RHO_RAND_TESTHAKEN", 0u)==6u&&kad_punkt>=2ull) { // ★ C2c Haken 6 (Pruefpass M1): die Ebene des VORIGEN Slice-Ereignisses lesen, BEVOR die neue geholt wird -> Soll: "VERALTET"
						print_warning("RHO_RAND-TESTHAKEN 6: lese rho auf der Slice-Ebene eines frueheren Zeitschritts.");
					...
					if(lbm_f.lbm_domain[0]->rho_rand_on&&env_u("CFD_RHO_RAND_TESTHAKEN", 0u)==8u) { // ★ C2c Haken 8 (Pruefpass NIEDRIG 4): R1-Zelle, aber NICHT gepflegt (y=1, weder TYPE_E noch x >= Nx-2) -> Soll: Sperre
						print_warning("RHO_RAND-TESTHAKEN 8: lese rho an einer ungepflegten R1-Zelle (y=1).");
						lbm_f.rho.get((ulong)(fNx/2u)+((ulong)1u+(ulong)(fNz/2u)*(ulong)fNy)*(ulong)fNx);
					}
				} else {
	...
	{
		const double wall_ges = std::chrono::duration<double>(t_now()-lauf_wall0).count();
		const double idx_ges  = t_si_letzt>0.0 ? wall_ges/(double)t_si_letzt : 0.0;
		print_info("[LEISTUNG-GESAMT] Wanduhr der Zeitschleife "+to_string((float)wall_ges,1u)+" s fuer "
			+to_string((float)(t_si_letzt*1e3),1u)+" ms physikalisch -> Index "+to_string((float)idx_ges,0u)+" s_wall/s_phys."
			" DIESE Zahl traegt den Aufbau NICHT (er liegt vor der Schleife), aber die Anwaermphase schon.");
		if(perf_phys0>=0.0) {
			const double wall_p = std::chrono::duration<double>(t_now()-perf_wall0).count();
			const double phys_p = (double)t_si_letzt - perf_phys0;
			const double idx_p  = phys_p>0.0 ? wall_p/phys_p : 0.0;
			const double mlups  = wall_p>0.0 ? (double)perf_zellschritte*1e-6/wall_p : 0.0;
			print_info("[LEISTUNG-AB-MARKE] ab t = "+to_string((float)(perf_phys0*1e3),1u)+" ms: "+to_string((float)wall_p,1u)
				+" s Wanduhr fuer "+to_string((float)(phys_p*1e3),1u)+" ms physikalisch -> Index "+to_string((float)idx_p,0u)
				+" s_wall/s_phys, Durchsatz "+to_string((float)mlups,0u)+" MLUPs, definiert als (ratio*N_nah + N_fern) je Aussenschritt durch die Wanduhr -- NICHT vergleichbar mit der Laufanzeige, die EINE Domaene und EINEN Schritt misst ("
	...
	{ // ★ 15.09.2026 Klemmen S0c: Restfenster seit dem letzten Sample lesen, dann Bericht (sigma aus ber_cd = cd_rest-Reihe nach Warmlauf)
	  ...
	  const KlemmUrteil un_ = berichte_klemmbilanz(kb_nah, "Nahfeld", units_fine, (double)q_inf*(double)A_ref, u_lat, fNy, fNz, sig_);
	  const KlemmUrteil uf_ = berichte_klemmbilanz(kb_fern, "Fernfeld", units_coarse, (double)q_inf*(double)A_ref, u_lat, cNy, cNz, sig_);
	  if(kb_nah.init) { // ★ Z2c: Budget je Domaene und gemeinsam (sigma immer aus dem Nahfeld)
	    const double sgz_ = block_sem(ber_cz, 4u);
	    klemm_budget_bewerten("Nahfeld", un_, sig_, sgz_, out_dir+"klemm_budget.csv", true);
	    klemm_budget_bewerten("Fernfeld", uf_, sig_, sgz_, out_dir+"klemm_budget.csv", false);
	    KlemmUrteil ug_; ug_.pruefbar = un_.pruefbar&&uf_.pruefbar; ug_.grund = un_.pruefbar ? uf_.grund : un_.grund; ug_.kappung = un_.kappung||uf_.kappung; ug_.n = un_.n;
	    ug_.dcd_j = fabs(un_.dcd_j)+fabs(uf_.dcd_j); ug_.dcz_j = fabs(un_.dcz_j)+fabs(uf_.dcz_j); ug_.dcd_m = fabs(un_.dcd_m)+fabs(uf_.dcd_m); ug_.dcd_m_brutto = un_.dcd_m_brutto+uf_.dcd_m_brutto; // Z2c Pruefpass M2: Summe der BETRAEGE (ungünstigster Fall, gegenlaeufige Domaenen heben sich nicht auf)
	    ug_.rund_cd = un_.rund_cd+uf_.rund_cd; ug_.rund_cd_j = un_.rund_cd_j+uf_.rund_cd_j; ug_.rund_cd_m = un_.rund_cd_m+uf_.rund_cd_m; ug_.rund_cz = un_.rund_cz+uf_.rund_cz;
	    klemm_budget_bewerten("Nah+Fern", ug_, sig_, sgz_, out_dir+"klemm_budget.csv", false);
	  } }
	// ★ Audit-Schleife 16.09.2026, Befund B5: DIESE DREI WAECHTER STANDEN VOR DEM KLEMMENBERICHT. Sie enden auf print_error -> exit(1)
	// (utilities.hpp), also riss ein Slice-/VTK-Fehlalarm den GESAMTEN Klemmenblock mit: KLEMM-BILANZ, KLEMM-HUELLEN, POSITIV, KLEMM-BUDGET,
	// die Restfenster der CSVs und das Abschlussurteil. Im Kanalfall stand der Bericht schon richtig VOR den Waechtern. Reihenfolge jetzt
	// ueberall gleich: erst messen und berichten, dann die Wirkpfad-Waechter.
	...
	if(slice_ns==0ull&&slice_dt>0.0f) { // ★ S3 (Diff-Pruefung H3): Wirkpfad der LEGACY-Uhr -- vorher hatte der Produktionspfad (CFD_SLICE_DT) keinen Zaehler
		const ulong kad_soll_dt=(ulong)floor((double)t_si_letzt/(double)slice_dt)+1ull;
		if(kad_punkt==0ull&&kad_soll_dt>0ull) print_error("SLICE-AUSGABE (CFD_SLICE_DT) aktiv, aber KEIN Slice-Punkt gefeuert -- stiller No-Op.");
		else print_info("[SLICES] Legacy-Uhr Wirkpfad: "+to_string(kad_punkt)+" Slices (Soll ~"+to_string(kad_soll_dt)+").");
	}
	...
	{ const ulong bqa_n=(ulong)lbm_f.lbm_domain[0]->rho_clamp_hits[117], bqa_c=(ulong)lbm_c.lbm_domain[0]->rho_clamp_hits[117]; // ★ S5: Aussparungen des ABSTAND-Scans (Slot 117)
	  print_info("BODEN_EQ_ABSTAND-Aussparungen (Slot 117, t%100): Nahfeld "+to_string(bqa_n)+", Fernfeld "+to_string(bqa_c)+".");
 ...
 // ★ 07.09. SISM-Wirkpfad Slot 126/127
		...
		if(env_u("CFD_FAC_ELIBB",0u)==1u&&(ulong)df->rho_clamp_hits[67]==0ull) print_warning("CFD_FAC_ELIBB=1 angefordert, aber Slot 67 = 0 -- die Blende wird nie erreicht (stiller No-Op? S4)."); // ★ S4: VOR dem Pur-Block, sonst unerreichbar (Diff-Pruefung B3)
		if(LBM_Domain::s_fac_elibb_pur) { // ★ Pruefbefund Messlogik-3 (2026-08-25): der Pur-Return sitzt VOR Slot 7 -- das alte Soll kannte den Pur-Modus nicht und print_error (=exit) toetete den projizierten Cd-Pfad. Pur-Soll: Slot 7 = 0, ELIBB-Wirkpfad (Slot 67) > 0.
			if(wz!=0ull) print_error("Pur-Arm: Slot 7 muesste 0 sein -- Return-Position verschoben?");
	...
	if(env_u("CFD_FERN_FACETTEN", 0u)>0u) { // ★ P8: Wirkpfad-Nachweis FERNFELD (Muster Nahfeld; das Grobgitter laeuft n_outer Schritte, Ereignis-Slots t%100-gesampelt)
		...
		print_info("Facetten-Wirkpfad Fernfeld (P8): "+to_string(wzc)+" (Soll "+to_string(sollc)+" mod 2^32), tau-Klemme "+to_string((ulong)dc->rho_clamp_hits[8])
			...
			+", sn-Klemme/Gate "+to_string((ulong)dc->rho_clamp_hits[16])+", alpha|beta3>ut "+to_string((ulong)dc->rho_clamp_hits[18])+" (PEMA/APG im Fernfeld AUS)"):string("")));
		if(env_u("CFD_FAC_ELIBB",0u)==1u&&dc->fac_elibb_on&&(ulong)dc->rho_clamp_hits[67]==0ull) print_warning("[Fernfeld] CFD_FAC_ELIBB=1 angefordert, aber Slot 67 = 0 -- die Blende wird nie erreicht (stiller No-Op? S4)."); // ★ S4: VOR dem Pur-Block, sonst unerreichbar (Diff-Pruefung B3)
...
static void main_setup_fernfeld() {
	...
	LBM_Domain::s_sgs_wandfrei = env_u("CFD_SGS_WANDFREI", 0u)>0u; LBM_Domain::s_sgs_guo = env_u("CFD_SGS_GUO", 1u)>0u; LBM_Domain::s_sgs_diag = env_u("CFD_SGS_DIAG", 0u)>0u; LBM_Domain::s_sgs_diag_ab = (ulong)env_schritte("CFD_SGS_DIAG_AB", 0u);
	...
	const ulong n_steps = (ulong)(t_end/dt + 0.5f);
	const uint  sample_every = max(1u, env_schritte("CFD_SAMPLE_EVERY", 100u));
...
void main_setup_facetten_test() {
	...
	{ // Arm B: FACETTEN=2 (reiner Tausch)
		LBM_Domain::s_facetten=true; LBM_Domain::s_fac_imem=false; LBM_Domain::s_fac_rdiag=0u; LBM_Domain::s_fac_ema=0.0f; LBM_Domain::s_fac_pema=0.0f; LBM_Domain::s_fac_satgate=false; LBM_Domain::s_fac_kraft = 0u; LBM_Domain::s_fac_kdiag = 0u; LBM_Domain::s_fac_nachbar = 0u; LBM_Domain::s_fac_messnur = 0u; LBM_Domain::s_sgs_fdwand = 0u; LBM_Domain::s_sgs_gdiag = 0u; LBM_Domain::s_sgs_sism = 0u; LBM_Domain::s_sgs_sism_T = 0u; LBM_Domain::s_sgs_sism_ab = 0ull; LBM_Domain::s_sgs_vandriest = 0u; LBM_Domain::s_sgs_band = 0u; LBM_Domain::s_sgs_nut_skal = 1.0f; LBM_Domain::s_sgs_vd_ab = 0ull; LBM_Domain::s_fac_alpha=0u; LBM_Domain::s_fac_apg=0.0f; LBM_Domain::s_boden_eq_n=0u; LBM_Domain::s_boden_eq_down=0u; LBM_Domain::s_boden_eq_split=0xFFFFFFFFu; LBM_Domain::s_boden_eq_abstand=0u; LBM_Domain::s_einlass_eq_n=0u; LBM_Domain::s_schale_alpha=0.0f; LBM_Domain::s_fac_diagz=-1l; LBM_Domain::s_fac_tau=0.0f;
	...
	// volle Ziel -twe modifiziert sofort, und die 1-Schritt-Lokalisierung (Auflage 3) bleibt exakt.
	{
		...
		LBM_Domain::s_facetten=true; LBM_Domain::s_fac_imem=true; LBM_Domain::s_fac_rdiag=0u; LBM_Domain::s_fac_ema=0.0f; LBM_Domain::s_fac_pema=0.0f; LBM_Domain::s_fac_satgate=false; LBM_Domain::s_fac_kraft = 0u; LBM_Domain::s_fac_kdiag = 0u; LBM_Domain::s_fac_nachbar = 0u; LBM_Domain::s_fac_messnur = 0u; LBM_Domain::s_sgs_fdwand = 0u; LBM_Domain::s_sgs_gdiag = 0u; LBM_Domain::s_sgs_sism = 0u; LBM_Domain::s_sgs_sism_T = 0u; LBM_Domain::s_sgs_sism_ab = 0ull; LBM_Domain::s_sgs_vandriest = 0u; LBM_Domain::s_sgs_band = 0u; LBM_Domain::s_sgs_nut_skal = 1.0f; LBM_Domain::s_sgs_vd_ab = 0ull; LBM_Domain::s_fac_alpha=0u; LBM_Domain::s_fac_apg=0.0f; LBM_Domain::s_boden_eq_n=0u; LBM_Domain::s_boden_eq_down=0u; LBM_Domain::s_boden_eq_split=0xFFFFFFFFu; LBM_Domain::s_boden_eq_abstand=0u; LBM_Domain::s_einlass_eq_n=0u; LBM_Domain::s_schale_alpha=0.0f; LBM_Domain::s_fac_diagz=-1l; LBM_Domain::s_fac_tau=1.0f;
